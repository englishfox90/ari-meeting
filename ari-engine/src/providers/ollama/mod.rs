...
use metadata::ModelMetadataCache;
...
/// Pure logic for the `pull_ollama_model` command — the host's
/// `#[tauri::command]` shim takes `tauri::State<'_, Arc<Engine>>` (which
/// deref-coerces to `&Engine`) and forwards here.
pub async fn pull_ollama_model_impl(
    engine: &Engine,
    model_name: String,
    endpoint: Option<String>,
) -> Result<(), String> {
    // Owned sink: used inside a `map_err` closure and across the streaming
    // loop below, so a borrow of `&Engine` won't do.
...
pub async fn get_ollama_embedding(
    ...
    let parsed: OllamaEmbeddingResponse = response
        .json()
        .await
        .map_err(|e| format!("could not parse embedding response: {e}"))?;

    if parsed.embedding.is_empty() {
        return Err(format!("embedding response was empty (is '{model}' pulled?)"));
    }
    Ok(parsed.embedding)
}
