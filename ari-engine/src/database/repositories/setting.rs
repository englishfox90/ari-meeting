...
impl SettingsRepository {
    ...
    /// Recall embedder selection ('apple' default | 'nomic-gguf' | 'ollama'). NULL/absent =
    /// 'apple'. Read by the recall indexer/search to pick how to embed.
    pub async fn get_recall_embedder(
        pool: &SqlitePool,
    ) -> std::result::Result<Option<String>, sqlx::Error> {
        let value: Option<Option<String>> =
            sqlx::query_scalar::<_, Option<String>>("SELECT recall_embedder FROM settings LIMIT 1")
                .fetch_optional(pool)
                .await?;
        Ok(value.flatten())
    }

    pub async fn save_recall_embedder(
        pool: &SqlitePool,
        embedder: &str,
    ) -> std::result::Result<(), sqlx::Error> {
        sqlx::query("UPDATE settings SET recall_embedder = ?")
            .bind(embedder)
            .execute(pool)
            .await?;
        Ok(())
    ...
    pub async fn save_api_key(
        ...
        let api_key_column = match provider {
            ...
            "claude-cli" => return Ok(()), // Uses local Claude CLI sign-in, no API key
            "apple-foundation" => return Ok(()), // On-device FoundationModels, no API key
            _ => {
                return Err(sqlx::Error::Protocol(
    ...
    pub async fn get_api_key(
        ...
        let api_key_column = match provider {
            ...
            "claude-cli" => return Ok(None), // Uses local Claude CLI sign-in, no API key
            "apple-foundation" => return Ok(None), // On-device FoundationModels, no API key
            _ => {
                return Err(sqlx::Error::Protocol(
                    format!("Invalid provider: {}", provider).into(),
                ))
    ...
    pub async fn save_transcript_api_key(
        ...
        let api_key_column = match provider {
            ...
            "apple" => return Ok(()), // Apple on-device STT needs no API key
    ...
    pub async fn get_transcript_api_key(
        ...
        let api_key_column = match provider {
            ...
            "apple" => return Ok(None), // Apple on-device STT needs no API key
            "deepgram" => "deepgramApiKey",
            "elevenLabs" => "elevenLabsApiKey",
            "groq" => "groqApiKey",
            "openai" => "openaiApiKey",
