//! The pure `DatabaseManager` — pool ownership, migrations, transactions,
//! cleanup. Moved verbatim out of the Tauri host crate (Stage B1 of the
//! ari-engine carve, `docs/plans/ari-engine-carve.md`).
//!
//! The `AppHandle`-taking constructors (`new_from_app_handle`, `is_first_launch`,
//! `import_legacy_database`) — which resolve `app_data_dir` before delegating to
//! [`DatabaseManager::new`] — stay host-side as free functions in
//! `frontend/src-tauri/src/database/manager.rs`, since this crate can never
//! depend on `tauri`.
...
impl DatabaseManager {
    pub async fn new(tauri_db_path: &str, backend_db_path: &str) -> Result<Self> {
        ...
        sqlx::migrate!("../frontend/src-tauri/migrations")
            .run(&pool)
            .await?;
