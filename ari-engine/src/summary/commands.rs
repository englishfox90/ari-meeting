//! Pure logic behind the `summary` command surface — split out of
//! `frontend/src-tauri/src/summary/commands.rs` (Phase 1.5 carve, Stage B1).
//! The `#[tauri::command]` shims (same fn names, `_impl` suffix dropped) stay
//! host-side and call straight into these.
...
pub async fn api_save_meeting_summary_impl(
    engine: &Engine,
    meeting_id: String,
...
pub async fn api_get_meeting_summary_language_impl(
    engine: &Engine,
    meeting_id: String,
...
pub async fn api_save_meeting_summary_language_impl(
    engine: &Engine,
    meeting_id: String,
...
pub async fn api_get_meeting_detected_summary_language_impl(
    engine: &Engine,
    meeting_id: String,
) -> Result<MeetingSummaryLanguagePreference, String> {
    log_info!(
...
pub async fn api_save_meeting_detected_summary_language_impl(
    engine: &Engine,
    meeting_id: String,
...
pub fn api_detect_transcript_summary_language_impl(
...
pub async fn api_get_summary_impl(
    engine: &Engine,
    meeting_id: String,
    ...
    match SummaryProcessesRepository::get_summary_data_for_meeting(pool, &meeting_id).await {
        Ok(Some(process)) => {
            ...
            // Fetch meeting title from database (metadata-only; avoids loading all transcripts)
        ...
        Ok(None) => {
            ...
            // Still fetch meeting title for idle state (metadata-only; avoids loading all transcripts)
            let meeting_name = match MeetingsRepository::get_meeting_metadata(pool, &meeting_id).await {
...
pub async fn api_process_transcript_impl(
    engine: &Engine,
    text: String,
    model: String,
    model_name: String,
...
pub async fn api_cancel_summary_impl(engine: &Engine, meeting_id: String) -> Result<serde_json::Value, String> {
