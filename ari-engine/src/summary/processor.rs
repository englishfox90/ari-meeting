use crate::summary::citations::apply_citations;
...
pub async fn generate_meeting_summary(
        ...
        resolve_cached_english(cached_english, summary_language)
    {
        ...
        if (provider != &LLMProvider::Ollama && provider != &LLMProvider::BuiltInAI && provider != &LLMProvider::AppleFoundation) || total_tokens < token_threshold {
            info!(
        ...
        info!("Summary pass completed ({} chars)", cleaned_markdown.len());

        // Deterministic citation post-processing: verify/snap/back-fill
        // @ref(...) timestamps against the ORIGINAL transcript (`text`),
        // which always carries real [MM:SS] markers even when the
        // map-reduce branch above summarized marker-less chunks. Pure,
        // panic-guarded so a bug here can never fail summary generation
        // (No-Fake-State cuts both ways: never invent a citation, but also
        // never let citation cleanup break the summary itself).
        let english_markdown = match std::panic::catch_unwind(|| {
            apply_citations(&cleaned_markdown, text)
        }) {
            Ok((rewritten, stats)) => {
                info!(
                    "✓ Citations: {} verified, {} snapped, {} dropped, {} back-filled",
                    stats.verified, stats.snapped, stats.dropped, stats.backfilled
                );
                rewritten
            }
            Err(_) => {
                error!("Citation post-processing panicked; using unmodified summary markdown");
                cleaned_markdown
            }
        };
