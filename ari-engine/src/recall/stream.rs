//! Streaming variant of `api_answer_meetings_locally` (F7) — `_impl` fn for the host's
//! `#[tauri::command]` shim (`frontend/src-tauri/src/recall/stream.rs`), moved during the
//! ari-engine carve (Stage B1, `docs/plans/ari-engine-carve.md`). Same retrieval, gating,
//! prompt, and citation-verification invariants as the single-shot path — they are
//! reused from `crate::recall` (the security-relevant gates and the anti-hallucination
...
//! events, then a terminal event carrying the citation-reconciled full answer plus the
//! (separately-computed, never model-invented) sources.
//!
...
pub async fn api_answer_meetings_locally_stream_impl(
    engine: &Engine,
    stream_id: String,
    question: String,
