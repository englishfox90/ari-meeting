//! Ask conversation persistence with a 7-day retention window — `_impl` fns for the
//! host's `#[tauri::command]` shims (`frontend/src-tauri/src/recall/conversations.rs`),
...
pub async fn ask_conversation_list_impl(
    engine: &Engine,
...
pub async fn ask_conversation_get_impl(
    engine: &Engine,
...
pub async fn ask_conversation_create_impl(
    engine: &Engine,
    meeting_id: Option<String>,
    title: Option<String>,
...
pub async fn ask_message_append_impl(
    engine: &Engine,
    conversation_id: String,
    role: String,
    content: String,
...
pub async fn ask_conversation_delete_impl(
    engine: &Engine,
    conversation_id: String,
) -> Result<(), String> {
    let db = engine.db().await?;
