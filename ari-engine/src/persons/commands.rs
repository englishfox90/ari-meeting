// Person Profiles (F2) + Owner Context (F3) — pure command logic. The `#[tauri::command]`
// shims (registered as `persons::commands::*` in `lib.rs`'s `generate_handler!` list) stay
// host-side in `frontend/src-tauri/src/persons/commands.rs` and call straight into these
// `*_impl` fns, per the ari-engine carve's per-service migration recipe
...
pub async fn person_list_impl(engine: &Engine) -> Result<Vec<PersonSummary>, String> {
...
pub async fn person_get_impl(engine: &Engine, person_id: String) -> Result<PersonDetail, String> {
    let db = engine.db().await?;
    let pool = db.pool();
...
pub async fn person_upsert_impl(engine: &Engine, person: NewPerson) -> Result<Person, String> {
    let db = engine.db().await?;
    let pool = db.pool();
...
pub async fn person_delete_impl(engine: &Engine, person_id: String) -> Result<(), String> {
    let db = engine.db().await?;
    let pool = db.pool();
...
pub async fn owner_get_impl(engine: &Engine) -> Result<Option<Person>, String> {
    let db = engine.db().await?;
    let pool = db.pool();
...
pub async fn owner_set_impl(engine: &Engine, person: NewPerson) -> Result<Person, String> {
    let db = engine.db().await?;
    let pool = db.pool();
...
pub async fn person_import_from_event_impl(
    engine: &Engine,
    event_id: String,
...
pub async fn meeting_participants_impl(
    engine: &Engine,
    meeting_id: String,
...
pub async fn profile_facts_for_person_impl(
    engine: &Engine,
    person_id: String,
...
pub async fn profile_facts_pending_impl(
    engine: &Engine,
...
pub async fn profile_fact_confirm_impl(engine: &Engine, fact_id: String) -> Result<(), String> {
    let db = engine.db().await?;
    let pool = db.pool();
...
pub async fn profile_fact_sources_impl(
    engine: &Engine,
    fact_id: String,
...
pub async fn profile_fact_reject_impl(engine: &Engine, fact_id: String) -> Result<(), String> {
    let db = engine.db().await?;
    let pool = db.pool();
...
pub async fn profile_fact_add_manual_impl(
    engine: &Engine,
    person_id: String,
    fact_text: String,
    fact_kind: String,
...
pub async fn person_extract_facts_for_meeting_impl(
    engine: &Engine,
    meeting_id: String,
...
/// lifecycle hook).
pub async fn person_reconcile_facts_for_meeting_impl(
    engine: &Engine,
    meeting_id: String,
...
pub async fn person_facts_needing_review_impl(
    engine: &Engine,
    person_id: String,
) -> Result<Vec<ProfileFact>, String> {
    let db = engine.db().await?;
...
/// Assembles the meeting-context block injected into the summary prompt (F2/F3/F4/F9).
///
/// **Host seam:** the organization name is company-wide config loaded from the Tauri
/// `AppHandle` (`crate::app_config::load`, a HOST-ONLY module — it can't move to
/// `ari-engine`, which has no `AppHandle`). The host shim resolves it BEFORE calling this
/// `_impl` and passes only the resolved `String` in, per the ari-engine carve's
/// `[client-side]`-style seam handling (`docs/plans/ari-engine-carve.md` "THE ONE HOST
/// SEAM"). Everything else here is pure DB/engine logic.
pub async fn summary_context_for_meeting_impl(
    engine: &Engine,
    organization: &str,
    meeting_id: String,
