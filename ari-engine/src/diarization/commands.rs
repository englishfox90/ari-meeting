...
async fn run_diarization(
    ...
    if mic_path.exists() {
        match process_mic_track(pool, engine, meeting_id, mic_path, scratch, seg_model, emb_model, emb_model_str).await
        {
    ...
    if let Some(src) = remote_src {
        ...
        let diar = engine::diarize(&wav_str, num_speakers, threshold, &seg_model.to_string_lossy(), &emb_model.to_string_lossy())
            .await
...
async fn process_mic_track(
    ...
    let diar: engine::DiarizeResult = match diar_res {
        ...
        other => {
            ...
            let centroid = engine::embed(&wav_str, &emb_model.to_string_lossy())
                .await
