...
//  `Ari` (macOS, full engine) and `Ari Lite` (iOS/iPadOS, read-only). Its intended
//  internal shape:
//
//      AriKit
...
//
//  STATUS: SCAFFOLD ONLY. Per the plan's WIP limits (principle 8), no engine code is
//  ported into here yet — Phase 0 spikes (S1–S4) gate that. This package exists now so
//  the Claude-Code Swift tooling (/swift-build, /swift-test, the PostToolUse SwiftLint/
//  SwiftFormat hook, XcodeBuildMCP's swift_package_build/test) has a real home to operate
//  on from day one, and so net-new Swift work has somewhere to land.
//
