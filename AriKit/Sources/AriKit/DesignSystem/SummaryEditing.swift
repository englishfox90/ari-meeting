...
//  `docs/plans/native-text-formatting.md` §3.3).
//
...
//
//  `context: Font.Context?` (no default — an omitted context is exactly the bug this exists to
//  fix, so it must be spelled at every call site) lets `SummaryCanonicalFont.emphasis` recover a
//  NATIVELY-applied emphasis (Font ▸ Bold/Italic) via Tier 2 before these toggle/carry it, so a
//  toolbar action never double-applies on top of, or drops, a native command's effect.
//
...
public enum SummaryEditing {
    ...
    public static func toggleBold(
        in text: inout AttributedString, selection: inout AttributedTextSelection, context: Font.Context?
    ) {
        text.transformAttributes(in: &selection) { container in
            let kind = container.summaryBlock ?? .paragraph
            let emphasis = SummaryCanonicalFont.emphasis(of: container.font, for: kind, context: context)
    ...
    /// so bold/italic — INCLUDING a natively-applied one recovered via Tier 2 — survives a
    /// heading↔body switch.
    ///
    ...
    public static func setBlockKind(
        ...
        text.transformAttributes(in: &selection) { container in
            ...
            let emphasis = SummaryCanonicalFont.emphasis(of: container.font, for: previousKind, context: context)
