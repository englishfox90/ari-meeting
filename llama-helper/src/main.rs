...
enum Request {
    Generate {
        ...
        /// When true, emit each decoded token as a `Token` line during generation,
        /// then the usual terminal `Response` carrying the full (stop-cleaned) text.
        /// Absent/false preserves the original single-shot behavior.
        stream: Option<bool>,
    },
    /// Embed one or more texts through an embedding-enabled context (mean-pooled). Loads
    /// `model_path` if provided (reusing the single-model slot). Used by the DEDICATED embed
    /// sidecar instance so it never contends with a summary process. `normalize` (default true)
    /// applies L2-normalization to each vector.
    Embed {
        texts: Vec<String>,
        model_path: Option<String>,
...
#[serde(tag = "type", rename_all = "snake_case")]
enum Response {
    /// Incremental token emitted only while `stream` is enabled (cosmetic; the
    /// terminal `Response` remains authoritative).
    Token { text: String },
    Response { text: String, error: Option<String> },
    /// One vector per input text, in order (mean-pooled, L2-normalized unless disabled).
...
/// In-place L2-normalization. Leaves an all-zero vector untouched (avoids NaN).
...
impl ModelState {
    ...
    fn generate(
        ...
        loop {
            ...
            // Stream this token to the driver (best-effort; the terminal Response
            // below carries the authoritative, stop-cleaned full text).
            if stream && !token_text.is_empty() {
                let _ = send_response(&Response::Token { text: token_text });
            }
    ...
    /// Produce a mean-pooled embedding for each input text. A single embedding-enabled
    /// context is created (sized to the longest input) and reused; the KV cache is cleared
    /// between texts so pooled vectors never bleed across inputs.
    fn embed(&mut self, texts: Vec<String>, normalize: bool) -> Result<Vec<Vec<f32>>> {
        let model = self.model.as_ref().context("Model not loaded")?;
        ...
        // Tokenize every input up front (nomic-bert expects a BOS token) so we can size the
        // batch/ubatch to the longest sequence — embedding decode requires n_batch >= n_tokens.
        let mut tokenized = Vec::with_capacity(texts.len());
        for text in &texts {
            let toks = model
                .str_to_token(text, AddBos::Always)
                .with_context(|| "failed to tokenize embed input")?;
            tokenized.push(toks);
        }
        ...
        // Context window must cover the longest sequence; batch/ubatch must fit all tokens
        // decoded in a single call (we decode one sequence at a time).
        let n_ctx = (max_tokens as u32).max(self.context_size);
        let n_batch = max_tokens as u32;

        let ctx_params = LlamaContextParams::default()
            .with_n_ctx(Some(
                NonZeroU32::new(n_ctx).context("Invalid embed ctx size")?,
            ))
        ...
        let mut ctx = model
            .new_context(&self.backend, ctx_params)
            .context("unable to create embedding context")?;
        ...
        for tokens in tokenized {
            ...
            batch
                ...
                .context("Failed to add sequence to embed batch")?;
            ctx.decode(&mut batch).context("embedding decode failed")?;

            let emb = ctx
                .embeddings_seq_ith(0)
                .context("failed to read pooled sequence embedding")?;
            let mut vector = emb.to_vec();
            if normalize {
            ...
            Ok(_) => {
                ...
                match serde_json::from_str::<Request>(line) {
                    ...
                    Ok(Request::Embed {
                        ...
                        // Load model if a path was provided (embedding contexts are created
                        // per request; the model slot is shared/idempotent).
                        if let Some(path_str) = model_path {
                            let path = PathBuf::from(path_str);
                            if let Err(e) = state.load_model_if_needed(path, state.context_size) {
                                send_response(&Response::Error {
                                    message: format!("Failed to load embed model: {}", e),
                                })?;
                                continue;
                            }
                        }

                        match state.embed(texts, normalize) {
                            Ok(vectors) => {
                                send_response(&Response::Embeddings { vectors })?;
                            }
                            Err(e) => {
                                send_response(&Response::Error {
...
mod tests {
    ...
    #[test]
    fn embed_request_deserializes_with_defaults() {
        let json = r#"{"type":"embed","texts":["hello","world"]}"#;
        let request: Request = serde_json::from_str(json).unwrap();
        let Request::Embed { texts, model_path, normalize } = request else {
    ...
    fn l2_normalize_leaves_zero_vector_untouched() {
        ...
        l2_normalize(&mut v);
        assert_eq!(v, vec![0.0f32, 0.0, 0.0]);
    }

    #[test]
    fn embeddings_response_serializes() {
        let resp = Response::Embeddings { vectors: vec![vec![0.6, 0.8]] };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"type\":\"embeddings\""));
        assert!(json.contains("\"vectors\":[[0.6,0.8]]"));
    }
