...
- Fold the Mac engine's models into the shared `AriKit` package so Mac and iOS share one definition (the sync-aware schema converges here — see Phase 3's checklist; CloudKit is still off at this point).
...
- **Store strategy: SQLiteData (Point-Free)** — **decision reaffirmed 2026-07-16 (Paul asked "isn't there a more native Apple store?").** The more-native option is **SwiftData** (Core Data successor), and it's rejected *specifically because* it hides the SQLite file — no raw SQL, no FTS5, no loadable extensions like sqlite-vec — which would amputate Ari's recall/Ask differentiator (F7 hybrid FTS5 ⊕ vector search, BM25⊕vector RRF). SQLiteData is **real SQLite** (recall port carries straight over — the Swift mirror of the repositories-only rule) *with* first-class CloudKit sync — "lean into Apple as far as possible without giving up raw SQL." Its **local** merits are confirmed in Phase 0; the CloudKit-sync half is validated at Phase 5.5. **Fallback if sync ever sours: GRDB + thinner hand-rolled sync — never SwiftData** (the recall constraint is invariant). Full reasoning in Phase 3 step 1. *(Was open Q1.)*
...
Phase 0  │ P0 commit eval rigs + spikes S1–S3  ── go/no-go; S4 moved to 5.5; S3 next   │
Phase 1  │ (collapsed)                         ── sync machinery → 5.5; schema-aware→P3 │
Phase 1.5│ Extract headless ari-engine         ── command audit (~140→~90) + protocol   │
Phase 2  │ Native SwiftUI shell                ── native read UI + scoped WebView       │
         │                                       + EventKit + notch UI + data import    │
Phase 3  │ Engine → Swift, store FIRST         ── SQLiteData store built SYNC-AWARE but  │
...
**Immediate next step given the local-only focus:** close out Phase 0 with **S3** (FluidAudio diarization parity — pure local Swift/CoreML, no account) and the pending **S1 human A/B**. Everything cloud is now parked behind Phase 5.5.
