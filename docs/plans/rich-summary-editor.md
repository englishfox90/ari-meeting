...
- Bold run → `**…**`, italic → `*…*`, both → `***…***`; runs coalesced so `**a****b**` never appears. **This coalescing rewrites the paragraph string, so it is deliberately NOT block-stable** (`parse` stores emphasis markers verbatim): `**a****b**` → `**ab**`, and a malformed odd-delimiter run like `*a**b*` → `*ab*`. Both are fixed points (tidy at most once), pinned by `acceptedEmphasisCanonicalization`, and the deferred backfill's verify-before-write (`parse`-equal) simply skips such a row rather than rewriting it. This is why `allFixtures` (test 20) contains only well-formed emphasis.
- **Two sibling lists of the same kind, blank-line separated, must not merge.** `present` has no representation of the blank line between two same-kind list *blocks*, and adjacent same-kind items serialize with a single `\n` (which re-parses as one merged list). Fix: `present` inserts an empty separator paragraph between adjacent same-kind list blocks, and `join` treats an empty piece as a boundary forcing a blank line — so `- a\n\n- b` stays two lists (test `adjacentSameKindListsDoNotMerge`, plus `allFixtures` coverage). Different-kind adjacent lists never merge (the parser's per-kind scan stops), so no separator there.
...
- **`SummaryInkConstraint`** — foreground color forced to the block's Marginalia ink role (headings `inkHeading`, body `inkBody`). **Step-5 must-do (review LOW):** `SummaryRichText.present` hardcodes the *light*-scheme ink (it has no `ColorScheme` param, and ink is irrelevant to the round-trip), so the presented document is wrong-colored in dark mode until this constraint re-applies the scheme ink. The constraint is what makes ink correct — verify it fires on first render in dark mode.
...
**Risks**
- **R1 — Cmd+B font identity. CLOSED 2026-07-24, see `docs/plans/native-text-formatting.md`.** Native bold toggling writes SwiftUI `font` values whose equality against our canonical `bodyBold` is not guaranteed. Confirmed exactly as predicted: native Font ▸ Bold/Italic replace `container.font` with a platform-font-boxed `Font` that fails Tier-1 value equality outright, so `SummaryFontConstraint` fell through to the plain base and silently dropped the emphasis (`docs/plans/native-text-formatting-findings.md`). Fixed with a Tier-2 resolve-based recovery (`Font.resolve(in:)`, keyed relative to the target kind's own base font) plus a minimal custom Format menu (`SummaryFormatCommands`, `CommandGroup(replacing: .textFormatting)`) binding ⌘B/⌘I, which were unbound entirely before this work. One inherent, pre-existing limitation remains and is not a regression: the two heading font families carry neither a bold nor an italic cut (two static Bricolage weights, no italic face at all), so native Font ▸ Bold/Italic on a heading is an AppKit-level no-op — nothing to recover. The toolbar/⌘B path still works on headings (it mints a distinct canonical `Font` *value* even though it renders identically), so markdown durability is unaffected; only the *native command* is inert there. `Font ▸ Underline/Highlight/Strikethrough` are stripped every constraint pass rather than left as dead checkmarks (No-Fake-State).
...
5. **LANDED 2026-07-24 (inspection-gated; NOT compiled).** `MarginaliaSummaryFormattingDefinition` (3 constraints: block-default, font coercion via the shared `SummaryFontVariant`, scheme ink) + `SummaryRichEditor` (chromeless per-segment `TextEditor(text:selection:)`, tables via `MarginaliaMarkdownView` no-handlers); swapped into `MeetingDetailView`: `summaryDraft: String` → `summaryDocument: SummaryEditDocument?`; `beginSummaryEdit` builds via `SummaryEditDocument.make`; `commitSummaryEdit` calls `viewModel.saveSummaryEdit(document.serialized())` with unchanged error handling; Save disabled via `isSerializedSummaryEmpty`; Cancel and the `.task(id: meetingId)` reset clear the document. `MarginaliaTextEditor` stays for Instructions/Notes. Spike deleted. `swift-code-reviewer` gate: **no BLOCKER/HIGH**; L3 (no first-render emphasis loss / round-trip preserved) CONFIRMED by logic. **Implemented on a Linux box with no Swift toolchain — never compiled or `swift test`ed here.** Three residuals need a Mac and are folded into Step 6 below (M1/L2/L4).
6. **Visual/manual checklist in the signed app — NOT YET RUN (needs a Mac; blocked in the implementing Linux environment).** Before this feature is "done":
   - **M1 (compile gate, do first):** `xcodebuild` must confirm `MarginaliaSummaryFormattingDefinition.swift` compiles — the three-constraint `@AttributedTextFormattingDefinitionBuilder body` is the idiomatic WWDC25-280 shape but the spike only proved a single constraint. If the builder rejects multiple, split into per-`AttributeKey` definitions.
   - **R1 / L2:** Cmd+B/Cmd+I actually coerce to canonical bold/italic (round-trips). If native toggling writes a non-canonical `Font`, `SummaryFontConstraint` flattens it to base → emphasis silently lost; fall back to `transformAttributes(in:&selection)` Cmd+B/Cmd+I shortcuts (API confirmed present).
   - **R2:** Enter in a bullet continues the list kind; typing at paragraph start keeps the kind.
   - **R4:** U+2028 caret/wrap behaves (else fall back to separate paragraphs, a D1 amendment).
   - **L4 (layout):** nested `TextEditor`-in-`ScrollView` height — verify tall summaries and table-bracketed multi-segment docs; single-editable (common) path should fill fine.
   - Rich paste from Safari coerced to the Marginalia set; tables inert in edit mode; read↔edit parity (same canvas/measure, no chrome) in light AND dark; Cancel/meeting-switch discard the draft; a write failure keeps the draft open.
