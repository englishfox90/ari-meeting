...
- **⇒ Next move — the 3 manager-state newtypes** (`ModelManagerState`, `EmbedModelManagerState`, `ParallelProcessorState`) that `engine/context.rs` depends on, then the `engine/` hub itself (Stage B1's remaining, load-bearing move).
