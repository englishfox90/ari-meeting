...
- **⇒ Remaining Stage B1 work** per the service map: audio pipeline proper, recall (embedding/search/chunker/agent), calendar (eventkit/sync/commands), persons, meeting_series (ledger/commands, `detection.rs` already moved), diarization, notch/apple bridges, ollama provider. None of these are blocked anymore — each can move independently, mirroring the established pattern (pure logic → crate, `#[tauri::command]`/`AppHandle` shim → host).
