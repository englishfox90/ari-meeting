...
- **⇒ Next move — the `engine/` hub itself** (`EventSink`/`Notifier`/`Paths`/`Engine`/`json_store`), the trait/impl split (Stage B1's last, load-bearing move — unblocks the ~37 dependent files).
