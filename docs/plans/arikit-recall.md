...
**SLICE 3 — Embedders behind `RecallEmbedder`. ✅ COMPLETE (2026-07-20).** Shipped Apple-only per plan: the `RecallEmbedder` protocol (`Sendable`, `embed(_:) async throws -> [[Float]]`, `modelTag`, + an `embedQuery` default extension) + `RecallEmbedderError` (`modelUnavailable`/`embeddingFailed`/`emptyResult`) + `AppleNLEmbedder` — in-process `NLEmbedding.sentenceEmbedding(for: .english)`, **NOT a sidecar** (a genuine simplification over Rust's apple-helper `embed_apple.rs`). Batch logic mirrors apple-helper's `Embed.run` verbatim: load once, embed each, `[Double]`→`[Float]`, throw honestly on any nil vector (No-Fake-State — never emit zeros), empty batch is a no-op. Swift-6-clean (stateless value type, genuinely `Sendable`; CPU/ANE work runs off the main actor by non-isolation). Tests: dimension/order/determinism/`modelTag` parity against the live model + a fake-conformer test for the `embedQuery` throw path. `swift-code-reviewer`-verified (no BLOCKER/HIGH/MEDIUM; 2 LOW addressed). 158 tests / 27 suites green. Next: Slice 4 (hybrid RRF search). `OllamaEmbedder` (loopback URLSession) still needs the ported provider layer; `MLXNomicEmbedder` (nomic GGUF) is still **blocked on the summary/Engine port** (Phase 3.4). Both gated, additive behind the protocol.

**SLICE 4 — Hybrid search (RRF). ✅ COMPLETE (2026-07-20, `327aa88`).** `HybridSearch.globalSearch`/`globalSearchScoped`: FTS5 BM25 arm ⊕ vector cosine arm, RRF fusion (`rrfK=60`, `1/(k+rank+1)`), recency half-life weighting (45d, floor 0.35), keyword-`LIKE` fallback added to the Store's `TranscriptRepository.searchTranscripts` (ported from `transcript.rs`). **Scoped path never leaks** (empty allowed / empty index / empty ranks all return `[]` — tested three ways); both arms **best-effort** (`try?`), semantic arm **never fabricates a vector** on embedder failure (No-Fake-State — gate-added degrade test proves lexical results still return without crash/fabrication). 15 tests (hand-computed RRF order, disjoint-arm recency isolation, scoped-no-leak, fallback, degrade, helper units). `swift-code-reviewer` PASS (parity + invariants confirmed). Built on Slice 2 (index) + Slice 3 (embedder).

**SLICE 5 — Indexer. ✅ COMPLETE (2026-07-20, `1a28039`).** `Indexer.indexMeeting` (fire-and-forget, logs-never-throws) / `reindexAll(force:)` guarded by a `ReindexCoordinator` **actor** (replaces Rust's module-level `AtomicBool` single-flight — a strict-concurrency win). FNV-1a content hash ported byte-for-byte (seed/prime); idempotency skips only when unchanged AND fully embedded with the current `modelTag`; embedding is **best-effort** (failure → lexical-only, never a partial/fabricated vector set). 9 tests (idempotent no-op, lexical→embedded upgrade, content-hash rebuild, force, concurrent single-flight). `swift-code-reviewer` PASS.

**SLICE 6 — Ask conversation store. ✅ COMPLETE (2026-07-20, `4b9ed1d`).** `AskConversationStore` (list/get/create/appendMessage) vended as `AppDatabase.askConversations`, over Slice 2's `askConversation`/`askMessage` tables. 7-day retention prune runs lazily on read/write (get() also prunes — a documented consistency delta); role gate refuses non-`user`/`assistant`; `sourcesJson` stores **app-supplied** `RecallSource`s as JSON, never parsed back from a model. **Independent of the LLM** — persistence only. 9 tests. `swift-code-reviewer` PASS.

**SLICE 7 — People context. ✅ COMPLETE — PARTIAL by design (2026-07-20, `65e3f35`).** `PeopleContext.peopleContextBlock` builds the owner + calendar-attendee + per-person top-fact block from persons/facts/calendar repos; bounded (`maxPeoplePerMeeting=8`, `maxFactChars=160`, `maxNoteChars=300`). **Diarization-derived speaker labels are GATED to Phase 3.5** — per No-Fake-State, `attachPeople` is an honest no-op (does not invent "who spoke"), marked `TODO(Phase 3.5)`. **Design note (flagged for product awareness):** the Rust participant-fact list reads a `meeting_participants` link table the Store lacks; this port substitutes a *real* signal — matching calendar attendees to `Person` rows by email (the same mechanism F2 uses) — rather than fabricating one. 8 tests. `swift-code-reviewer` PASS.

**SLICE 8 — Orchestrator (single-shot + streaming). ✅ SINGLE-SHOT + STREAMING DONE (2026-07-20); AGENTIC LOOP DEFERRED.** `RecallEngine.answerMeetingsLocally` (`Recall/Orchestrator/RecallEngine.swift`) ties `HybridSearch` (Slice 4) + the pure shell (Slice 1) + `PeopleContext` (Slice 7) + an `LLMClient` (Engine Phase 3.4, now landed) together, porting `shell.rs::api_answer_meetings_locally_impl` — question validation, the loopback gate, meeting/series/global retrieval (precedence **meeting > series > global**, ← `shell.rs:351`), series-ledger prompt injection (read-only via `SeriesRepository.find`), and citation/`@ref` reconciliation on the model's answer (never on what the model claims to have cited). `RecallStream.answerMeetingsLocallyStream` (`Recall/Orchestrator/RecallStream.swift`) is the streaming counterpart (← `stream.rs`), yielding `RecallStreamEvent.delta`/`.done` over a native `AsyncThrowingStream` (the resolved §9(6) decision) instead of the Rust `EventSink` event pair; both share one `prepare`/`reconcile` implementation so the gates are single-sourced, not copied. A new `RecallSettingsReading`/`RecallSecretsReading` protocol pair (`Recall/Orchestrator/RecallSettings.swift`, distinct from `Engine`'s own `SettingsReading`/`SecretsReading` — recall reads the CURRENTLY CONFIGURED provider/model, `SummaryService` receives it as an explicit argument) is the injected settings seam, with a `#if DEBUG` stub. **The Claude agentic tool-use loop is explicitly DEFERRED** (`agent.rs`, cloud-only, 5 tools, ≤8 iterations, `MAX_SOURCES=24`, `MAX_TRANSCRIPT_CHARS=8_000`) — a `// TODO(follow-on)` marks the gap in `RecallEngine.swift`; behaviorally this is a strict subset of Rust (this port always takes the single-shot/streaming path Rust falls back to on any agentic error), never worse, just less agentic reach for global Claude asks. 17 tests (`RecallEngineTests`): citation verification + `@ref` scoping (meeting keeps in-range, global strips all), sources built independently of the model's own text, the loopback gate rejected end-to-end (single-shot AND streaming), all question/model/no-match error messages, series-ledger injection, and the streaming delta/terminal-`.done` contract. 379 tests / 53 suites green.
