...
**SLICE 2 — Index schema + `RecallIndexRepository`.** The five tables (§4) as a Store migration + records + the repository (map of `recall_index.rs`) + `RecallChunk`/`RecallIndexState` domain values. **Depends on: Store (`AppDatabase`, `meeting` table for FKs).** Coordinate migration placement with the Store stream (§8).
