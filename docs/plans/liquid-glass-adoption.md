...
- **Tab/section switchers are stock segmented controls** (`Picker` + `.pickerStyle(.segmented)`, per the HIG tab-views guidance) — connected Liquid Glass tabs from the system, never a row of loose buttons. `MarginaliaSegmentedControl` is a gallery-only relic; don't use it in product screens (owner direction 2026-07-21, meeting-detail narrow switcher).
