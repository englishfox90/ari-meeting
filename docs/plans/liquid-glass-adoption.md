...
- **Tab/section switchers are `MarginaliaGlassTabs`** — the HIG capsule glass tab appearance (glass `Capsule` container + sliding selection-wash pill), which the stock macOS segmented `Picker` does not render (it draws the rectangular control). Never a row of loose buttons. `MarginaliaSegmentedControl` is a gallery-only relic; don't use either it or a raw segmented `Picker` for section tabs in product screens (owner direction 2026-07-21, meeting-detail narrow switcher).
