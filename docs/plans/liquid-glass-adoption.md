...
- **Tab/section switchers go in the TOOLBAR: `ToolbarItem(placement: .principal)` + segmented `Picker`.** The toolbar is the system's glass layer on macOS 26, so the switcher renders in the floating glass grouping there (the Finder/Notes/Music view-switcher idiom). Hard-won findings from 2026-07-21 (meeting-detail narrow switcher — don't re-tread): an **in-content** segmented `Picker` draws the flat rectangular control; an **embedded mid-layout `TabView`** draws a compact flat bar, not the glass tab bar; hand-built glass imitations (`MarginaliaGlassTabs`, deleted) are banned; `MarginaliaSegmentedControl` is gallery-only. Glass comes from the chrome layer, not from the control.
