...
- **Tab/section switchers are stock `TabView` + `Tab`** — on macOS 26 it renders the system's Liquid Glass tab bar (the HIG tab-views appearance) natively. Never a row of loose buttons, a raw segmented `Picker`, or a hand-built glass imitation (two were tried and deleted on 2026-07-21: the segmented-Picker attempt and `MarginaliaGlassTabs`; `MarginaliaSegmentedControl` remains gallery-only). Owner direction, meeting-detail narrow switcher.
