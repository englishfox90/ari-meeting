...
3. **Format auto-detection:** `ToolCallFormat.infer` maps `qwen3_5*` → `.xmlFunction` (`Tool/ToolCallFormat.swift:209-212`); plain `qwen3` (our Qwen/Qwen3-4B-MLX-4bit) falls through to the **default `.json` Hermes parser** (`<tool_call>{"name":…,"arguments":…}</tool_call>`, `ToolCallFormat.swift:64-67,111-112`) — exactly what Qwen3-4B is natively trained on.
