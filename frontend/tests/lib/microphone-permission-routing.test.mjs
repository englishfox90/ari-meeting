...
// The native recording call is wrapped in startBackendRecording, which owns the
// permission-wait hint timer and delegates to the recording service.
assert.match(source, /const startBackendRecording = useCallback/);
...
for (const entryPoint of [
  ...
  const recording = source.indexOf('startBackendRecording(', start);
