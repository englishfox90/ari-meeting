...
test('global shell uses the documented Arivo amber visual system', async () => {
  const [css, sidebar, mainContent, pageHeader, homePage, themeContext, themeControl, meetingPage, meetingTranscript, meetingSummary, preRecording, postRecording, designMarkdown, designJson] = await Promise.all([
  ...
  // Transcript panel is drag-resizable at xl: width is driven by a CSS var
  // that defaults to the original 22rem when the user hasn't resized.
  assert.match(meetingPage, /xl:static xl:z-auto xl:w-\[var\(--transcript-w,22rem\)\]/);
  assert.match(meetingPage, /ResizeHandle/);
