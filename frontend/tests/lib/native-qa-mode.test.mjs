...
test('native QA modes isolate data and never auto-start model downloads', async () => {
  ...
  // One NativeQA guard per download-start effect: parakeet, whisper, apple speech
  // assets, and the builtin-ai summary (the 3-tier onboarding download flow).
  assert.equal((downloadStep.match(/if \(isNativeQaMode\) return;/g) ?? []).length, 4);
