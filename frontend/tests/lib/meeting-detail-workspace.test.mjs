...
test('meeting detail remains a summary-first reading workspace with a persistent local inspector', async () => {
  ...
  // Transcript panel width is drag-resizable at xl via a CSS var (default 22rem).
  assert.match(content, /xl:static xl:z-auto xl:w-\[var\(--transcript-w,22rem\)\]/);
  ...
  assert.match(transcript, /aria-label="Meeting inspector"/);
