...
test('recording command treats omitted selected devices as system defaults', async () => {
  ...
  // Readiness must enumerate real devices (including outputs) rather than
  // fabricating a mic-only list — otherwise the panel reports system audio as
  // unavailable while a recording actually taps the default output (No-Fake-State).
  assert.doesNotMatch(readinessHook, /Promise\.resolve\(\[\{ name: 'Default microphone'/);
  assert.match(readinessHook, /invoke<AudioDevice\[\]>\('get_audio_devices'\)/);
  assert.equal(
