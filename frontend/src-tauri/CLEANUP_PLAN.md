...
> **Status (2026-07-16):** Phase 1 dead-code removal is **done** — `lib_old_complex.rs`, `audio/core-old.rs`, `audio/recording_saver_old.rs`, `audio/recording_commands.rs.backup`, `audio/stt.rs`, and the whole `audio_v2/` tree have been deleted (all were confirmed dead — undeclared in `lib.rs`, wired to no command). The former additive-only rule is retired; we now own the tree and remove dead code by default (see `.claude/rules/codebase-ownership.md`). Phases 2–4 (module dedup, dependency audit, quality) remain open.
