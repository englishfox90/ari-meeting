...
async fn run_retranscription<R: Runtime>(
    ...
    let whisper_engine = if !use_parakeet && !use_apple {
    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        let (text, conf) = if use_apple {
            // Apple SpeechAnalyzer via the apple-helper sidecar (spawn-per-segment).
            // "auto"/None → sidecar resolves to Locale.current (matches install/probe).
            let locale = language.clone().unwrap_or_else(|| "auto".to_string());
            let (text, confidence) = crate::apple::helper::transcribe(&segment.samples, &locale)
                .await
                .map_err(|e| anyhow!("Apple transcription failed on segment {}: {}", i, e))?;
    ...
    // Record the STT provider/model that actually ran this retranscription
    // as the meeting's transcription provenance.
    sqlx::query(
        "UPDATE meetings SET transcription_provider = ?, transcription_model = ?, updated_at = ? WHERE id = ?",
    )
    .bind(&provider)
    .bind(&model)
    .bind(chrono::Utc::now())
    ...
    .execute(&mut *tx)
    .await
    .map_err(|e| anyhow!("Failed to update meeting provenance: {}", e))?;
