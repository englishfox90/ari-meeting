...
pub async fn validate_transcription_model_ready<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    ...
    match config.provider.as_str() {
        ...
        "apple" => {
            // Apple SpeechAnalyzer uses the on-device system model — there is no
            // local model file to validate here. Real availability (OS support,
            // Speech assets installed) is surfaced honestly via `apple_probe` at
            // selection time, not gated at record-start.
            info!("🍎 Apple on-device transcription selected — no local model to validate");
            Ok(())
        }
        other => {
...
pub async fn get_or_init_transcription_engine<R: Runtime>(
    ...
    match config.provider.as_str() {
        ...
        "apple" => {
            info!("🍎 Initializing Apple on-device transcription engine");
            Ok(TranscriptionEngine::Provider(std::sync::Arc::new(
                crate::audio::transcription::apple_provider::AppleTranscriptionProvider::new(),
            )))
        }
        "localWhisper" | _ => {
