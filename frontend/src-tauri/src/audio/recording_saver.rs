...
/// Senders returned by `start_accumulation`: one per persisted audio track.
///
/// `mixed` feeds the historical `audio.mp4`; `mic`/`system` feed the net-new
/// separate `mic.m4a` / `system.m4a` tracks (needed for diarization + owner-voice
/// enrollment). All three are fire-and-forget from the pipeline's perspective.
pub struct RecordingSenders {
    pub mixed: mpsc::UnboundedSender<AudioChunk>,
    pub mic: mpsc::UnboundedSender<AudioChunk>,
    pub system: mpsc::UnboundedSender<AudioChunk>,
}

/// New recording saver using incremental saving strategy
pub struct RecordingSaver {
    incremental_saver: Option<Arc<AsyncMutex<IncrementalAudioSaver>>>,
    // NET-NEW: separate mic (owner) + system (remote) track savers, sharing the
    // same meeting_folder/.checkpoints/ as the mixed saver (distinct file stems).
    ...
    mic_chunk_receiver: Option<mpsc::UnboundedReceiver<AudioChunk>>,
    system_chunk_receiver: Option<mpsc::UnboundedReceiver<AudioChunk>>,
...
impl RecordingSaver {
    pub fn new() -> Self {
        Self {
            ...
            chunk_receiver: None,
            mic_chunk_receiver: None,
            system_chunk_receiver: None,
    ...
    pub fn start_accumulation(&mut self, auto_save: bool) -> RecordingSenders {
        if auto_save {
        ...
        // Create channel for receiving audio chunks (mixed track — existing behavior)
        let (sender, receiver) = mpsc::unbounded_channel::<AudioChunk>();
        self.chunk_receiver = Some(receiver);

        // NET-NEW: separate mic + system track channels (mirror the mixed channel)
        let (mic_sender, mic_receiver) = mpsc::unbounded_channel::<AudioChunk>();
        self.mic_chunk_receiver = Some(mic_receiver);
        let (system_sender, system_receiver) = mpsc::unbounded_channel::<AudioChunk>();
        self.system_chunk_receiver = Some(system_receiver);
        ...
        // NET-NEW: spawn identical drain loops for the mic + system track savers.
        // Same is_saving gate; fully independent of the mixed path (fail-safe).
        if let Some(receiver) = self.mic_chunk_receiver.take() {
            Self::spawn_track_drain(
                receiver,
                self.is_saving.clone(),
                self.mic_saver.clone(),
                auto_save,
                "mic",
            );
        }
        if let Some(receiver) = self.system_chunk_receiver.take() {
            Self::spawn_track_drain(
                receiver,
                self.is_saving.clone(),
                self.system_saver.clone(),
                auto_save,
                "system",
            );
        }
    ...
    /// Drain loop for a separate (mic/system) track saver. Mirrors the mixed-audio
    /// accumulation task: gated by the shared `is_saving` flag, only writes when
    /// `save_audio` is set, and never panics — a failing separate track must never
    /// take down the mixed recording.
    fn spawn_track_drain(
        ...
        tokio::spawn(async move {
            ...
            while let Some(chunk) = receiver.recv().await {
                let should_continue = if let Ok(is_saving) = is_saving.lock() {
                    *is_saving
                } else {
                    false
                };
                ...
                if save_audio {
                    if let Some(saver_arc) = &saver {
                        let mut saver_guard = saver_arc.lock().await;
                        if let Err(e) = saver_guard.add_chunk(chunk) {
                            error!("Failed to add chunk to [{}] saver: {}", label, e);
                        }
                    }
                    // If the saver failed to initialize, silently drop — mixed path is unaffected.
                }
            }

            info!("Recording saver [{}] accumulation task ended", label);
        });
    }
    ...
    pub fn discard_unstarted_recording(&mut self) {
        ...
        self.mic_chunk_receiver = None;
        self.system_chunk_receiver = None;
        self.incremental_saver = None;
        self.mic_saver = None;
        self.system_saver = None;
    ...
    fn folder_contains_recoverable_data(folder: &std::path::Path) -> bool {
        if folder.join("audio.mp4").is_file()
            || folder.join("mic.m4a").is_file()
            || folder.join("system.m4a").is_file()
    ...
    fn initialize_meeting_folder(
        ...
        if create_checkpoints {
            ...
            // NET-NEW: separate mic + system track savers, sharing the same
            // .checkpoints/ dir (distinct file stems → mic.m4a / system.m4a).
            // Fail-safe: if either fails to construct, log and continue —
            // the mixed audio.mp4 path above is already initialized and must not
            // be blocked by separate-track setup.
            match IncrementalAudioSaver::new_with_prefix(meeting_folder.clone(), 48000, "mic.m4a") {
                Ok(saver) => self.mic_saver = Some(Arc::new(AsyncMutex::new(saver))),
                Err(e) => warn!("Failed to init mic-track saver (mixed recording unaffected): {}", e),
            }
            match IncrementalAudioSaver::new_with_prefix(meeting_folder.clone(), 48000, "system.m4a") {
                Ok(saver) => self.system_saver = Some(Arc::new(AsyncMutex::new(saver))),
                Err(e) => warn!("Failed to init system-track saver (mixed recording unaffected): {}", e),
            }
        } else {
    ...
    pub async fn stop_and_save<R: Runtime>(
        ...
        // NET-NEW: finalize the separate mic + system tracks. These are best-effort:
        // the mixed audio.mp4 (finalized above) is the canonical recording, so a
        // failure to merge a separate track must NOT fail the save.
        if let Some(saver_arc) = &self.mic_saver {
            let mut saver = saver_arc.lock().await;
            match saver.finalize().await {
                Ok(path) => info!("✅ Finalized mic track: {}", path.display()),
                Err(e) => warn!("Failed to finalize mic track (non-fatal): {}", e),
            }
        }
        if let Some(saver_arc) = &self.system_saver {
            let mut saver = saver_arc.lock().await;
            match saver.finalize().await {
                Ok(path) => info!("✅ Finalized system track: {}", path.display()),
                Err(e) => warn!("Failed to finalize system track (non-fatal): {}", e),
            }
        }
