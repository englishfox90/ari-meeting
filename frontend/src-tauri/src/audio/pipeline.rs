...
pub struct AudioPipeline {
    ...
    // NET-NEW: separate-track recording senders (mic = owner, system = remote).
    // Forked off BEFORE mixing so mic/system stay independently persistable.
    recording_sender_for_mic: Option<mpsc::UnboundedSender<AudioChunk>>,
    recording_sender_for_system: Option<mpsc::UnboundedSender<AudioChunk>>,
}
...
impl AudioPipeline {
    pub fn new(
        ...
        Self {
            ...
            recording_sender_for_mic: None,    // NET-NEW: set by manager
            recording_sender_for_system: None, // NET-NEW: set by manager
        }
    }
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        loop {
            ...
            match tokio::time::timeout(
                ...
                Ok(Some(chunk)) => {
                    ...
                    // NET-NEW: publish an instantaneous level for the live meter (the
                    // notch HUD samples this lock-free at ~10 Hz). We use PEAK — not
                    // window RMS — because the mic is EBU R128 loudness-normalized to
                    // ~-23 LUFS, which flattens RMS so window-averaged RMS barely moves
                    // as someone speaks. Peak keeps speech's crest factor, so the meter
                    // actually tracks talking. A peak-hold decay (fast attack / gradual
                    // release) both smooths the display AND stops flicker between the
                    // alternating mic (loud) and system (often silent) chunks: a silent
                    // system chunk decays the held mic peak instead of dropping to zero.
                    // Cheap: one pass for max + a lone atomic store — never blocks the loop.
                    if !chunk.data.is_empty() {
                        let peak = chunk.data.iter().fold(0.0f32, |m, &x| m.max(x.abs()));
                        // ×1.3 fills the meter for normal speech without pinning at 1.0.
                        let instant = (peak * 1.3).min(1.0);
                        // Attack: jump up immediately on a louder peak. Release: decay the
                        // held value ~15%/chunk when the input is quieter. Tune here if the
                        // meter falls too fast/slow or reads hot/cold.
                        let level = instant.max(crate::audio::live_level::current() * 0.85);
                        crate::audio::live_level::publish(level);
                    }
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            // NET-NEW: fork the SEPARATE mic + system windows for track
                            // persistence BEFORE mix_window() collapses them irreversibly.
                            // Fire-and-forget (mirrors the mixed fork below); never blocks
                            // the pipeline loop and never affects the mixed recording.
                            if let Some(ref sender) = self.recording_sender_for_mic {
                                ...
                                let _ = sender.send(mic_chunk);
                            }
                            if let Some(ref sender) = self.recording_sender_for_system {
                                ...
                                let _ = sender.send(system_chunk);
                            }
        ...
        // NET-NEW: the pipeline is stopping — clear the live meter level so a
        // stale value can't linger into the next recording's first window.
        crate::audio::live_level::reset();
...
impl AudioPipelineManager {
    ...
    pub fn start(
        ...
        sample_rate: u32,
        recording_sender: Option<mpsc::UnboundedSender<AudioChunk>>,
        recording_sender_mic: Option<mpsc::UnboundedSender<AudioChunk>>,
        recording_sender_system: Option<mpsc::UnboundedSender<AudioChunk>>,
        mic_device_name: String,
        mic_device_kind: super::device_detection::InputDeviceKind,
        system_device_name: String,
        system_device_kind: super::device_detection::InputDeviceKind,
        ...
        // NET-NEW: wire the separate mic + system track senders
        pipeline.recording_sender_for_mic = recording_sender_mic;
        pipeline.recording_sender_for_system = recording_sender_system;
