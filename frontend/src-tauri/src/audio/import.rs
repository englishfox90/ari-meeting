...
async fn run_import<R: Runtime>(
    ...
    let whisper_engine = if !use_parakeet && !use_apple && total_segments > 0 {
...
async fn create_meeting_with_transcripts(
    ...
    // Insert meeting
    sqlx::query(
        "INSERT INTO meetings (id, title, created_at, updated_at, folder_path, transcription_provider, transcription_model)
         VALUES (?, ?, ?, ?, ?, ?, ?)",
    )
    .bind(&meeting_id)
    .bind(title)
