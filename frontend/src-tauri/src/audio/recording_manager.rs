...
impl RecordingManager {
    ...
    pub async fn start_recording(
        ...
        let recording_senders = self.recording_saver.start_accumulation(auto_save);

        // Start recording state first
        if let Err(error) = self.state.start_recording() {
            drop(recording_senders);
            self.recording_saver.discard_unstarted_recording();
            return Err(error);
        }
        ...
        if let Err(error) = self.pipeline_manager.start(
            ...
            Some(recording_senders.mixed), // CRITICAL: Pass recording sender to receive pre-mixed audio
            Some(recording_senders.mic),   // NET-NEW: separate mic (owner) track
            Some(recording_senders.system),// NET-NEW: separate system (remote) track
            mic_name,
            mic_kind,
            sys_name,
            sys_kind,
