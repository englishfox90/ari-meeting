...
pub struct IncrementalAudioSaver {
    ...
    sample_rate: u32,
    /// Stem used for per-track checkpoint files (`{stem}_chunk_{:03}.mp4`) so that
    /// multiple tracks (mixed "audio", "mic", "system") can share one `.checkpoints/`
    /// dir without colliding. Defaults to "audio".
    output_stem: String,
    /// Final merged filename written into the meeting folder (e.g. "audio.mp4",
    /// "mic.m4a", "system.m4a"). Defaults to "audio.mp4".
    output_filename: String,
}

impl IncrementalAudioSaver {
    /// Create a new incremental saver (mixed-audio path — writes `audio.mp4`).
    ///
    /// Byte-for-byte backward compatible: delegates to `new_with_prefix` with the
    /// legacy output filename "audio.mp4" (checkpoints `audio_chunk_*.mp4`).
    ///
    /// # Arguments
    ...
    pub fn new(meeting_folder: PathBuf, sample_rate: u32) -> Result<Self> {
        Self::new_with_prefix(meeting_folder, sample_rate, "audio.mp4")
    }

    /// Create a new incremental saver with an explicit output filename.
    ///
    /// The checkpoint stem is derived from the filename stem, so the mixed track
    /// ("audio.mp4") keeps its historical `audio_chunk_*.mp4` checkpoint names while
    /// new tracks ("mic.m4a", "system.m4a") use `mic_chunk_*` / `system_chunk_*`.
    ///
    /// # Arguments
    /// * `meeting_folder` - Path to the meeting folder (contains .checkpoints/)
    /// * `sample_rate` - Sample rate of audio (typically 48000)
    /// * `output_filename` - Final merged filename (e.g. "audio.mp4", "mic.m4a")
    pub fn new_with_prefix(meeting_folder: PathBuf, sample_rate: u32, output_filename: &str) -> Result<Self> {
        ...
        // Derive the checkpoint stem from the filename (e.g. "audio.mp4" -> "audio").
        let output_stem = std::path::Path::new(output_filename)
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("audio")
            .to_string();
    ...
    fn save_checkpoint(&mut self) -> Result<()> {
        ...
        // Generate checkpoint filename (per-track stem avoids collision in shared .checkpoints/)
        let checkpoint_path = self.checkpoints_dir
            .join(format!("{}_chunk_{:03}.mp4", self.output_stem, self.checkpoint_count));
    ...
    pub async fn finalize(&mut self) -> Result<PathBuf> {
        ...
        // Clean up ONLY this saver's own checkpoint files + concat list.
        //
        // The `.checkpoints/` dir is SHARED across the three tracks ("audio", "mic",
        // "system" — see `recording_saver.rs`), distinguished only by `{stem}_*`
        // filenames. A `remove_dir_all` here would delete the other two tracks'
        // in-flight checkpoints too (whichever saver finalizes first wins, and the
        // others silently fail to find their chunks). So we only ever remove files
        // that carry OUR `output_stem` prefix, then try to remove the directory
        // itself (ignored if it's not empty — another saver may still own it).
        info!("Cleaning up {} checkpoint files for stem '{}'", self.checkpoint_count, self.output_stem);
        self.cleanup_own_checkpoint_files();
    ...
    /// Remove only this saver's own `{stem}_chunk_*` checkpoint files and its own
    /// `{stem}_concat_list.txt`, then remove the shared `.checkpoints/` dir if it's
    /// now empty (another track's saver may still have files in it — that's fine,
    /// `remove_dir` fails harmlessly and we leave cleanup to whichever saver finishes
    /// last).
    fn cleanup_own_checkpoint_files(&self) {
        let prefix = format!("{}_chunk_", self.output_stem);

        match std::fs::read_dir(&self.checkpoints_dir) {
            Ok(entries) => {
                for entry in entries.filter_map(|e| e.ok()) {
                    ...
                    if is_ours {
                        if let Err(e) = std::fs::remove_file(&path) {
                            warn!("Failed to remove checkpoint file {}: {}", path.display(), e);
                        }
                    }
                }
            }
            Err(e) => {
                warn!("Failed to read checkpoints directory for cleanup: {}", e);
            }
        }

        // Remove our own concat list file too (merge_checkpoints wrote it).
        let list_file = self.checkpoints_dir.join(format!("{}_concat_list.txt", self.output_stem));
        if list_file.exists() {
            if let Err(e) = std::fs::remove_file(&list_file) {
                warn!("Failed to remove concat list file {}: {}", list_file.display(), e);
            }
        }

        // Best-effort: remove the shared dir if we just emptied it. Ignore errors —
        // a non-empty dir (another track still has files) or a benign race with
        // another saver's own cleanup are both expected, non-fatal outcomes.
        let _ = std::fs::remove_dir(&self.checkpoints_dir);
    }
    ...
    async fn merge_checkpoints(&self, output: &PathBuf) -> Result<()> {
        ...
        // Create concat list file for FFmpeg. Per-stem name avoids a collision/race
        // with the other tracks' savers (mixed "audio", "mic", "system") writing
        // into the same shared `.checkpoints/` dir concurrently.
        let list_file = self.checkpoints_dir.join(format!("{}_concat_list.txt", self.output_stem));
        let mut list_content = String::new();

        for i in 0..self.checkpoint_count {
            let checkpoint_path = self.checkpoints_dir
                .join(format!("{}_chunk_{:03}.mp4", self.output_stem, i));
...
pub async fn recover_audio_from_checkpoints(
    ...
    let mut checkpoint_files: Vec<_> = std::fs::read_dir(&checkpoints_dir)
        ...
        .filter(|entry| {
            // Only the MIXED track's checkpoints (`audio_chunk_*.mp4`) belong in the
            // recovered `audio.mp4`. The separate mic/system tracks now share this
            // `.checkpoints/` dir (`mic_chunk_*` / `system_chunk_*`); without this
            // prefix guard, recovery would concat all three tracks into one corrupt file.
            let path = entry.path();
            path.extension().and_then(|s| s.to_str()) == Some("mp4")
                && path
                    .file_name()
                    .and_then(|s| s.to_str())
                    .is_some_and(|n| n.starts_with("audio_chunk_"))
        })
        .collect();
