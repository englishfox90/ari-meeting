...
impl AudioStream {
    ...
    async fn create_cpal_stream(
        ...
        info!("🎤 CPAL: capture processor created for '{}', building stream...", device.name);

        // Build the appropriate stream based on sample format
        let stream = Self::build_stream(&cpal_device, &config, capture.clone())?;
        info!("🎤 CPAL: stream built for '{}', calling play()...", device.name);
...
impl AudioStreamManager {
    ...
    pub async fn start_streams(
        ...
        // Start the microphone (CPAL) BEFORE the system-audio tap.
        //
        // cpal's macOS backend starts the input AudioUnit during
        // `build_input_stream`, and that start DEADLOCKS in the CoreAudio HAL
        // (`HALB_IOThread::StartAndWaitForState` → mutex wait) when the
        // system-audio process-tap aggregate device is already running — two HAL
        // IO threads contend on the same guard and the mic start never completes
        // (confirmed via stack sample). Bringing the microphone up first, before
        // the tap exists, avoids that contention. The system-audio path below is
        // timeout-bounded internally (`run_blocking_audio_setup`), so if anything
        // blocks there it degrades to mic-only instead of hanging startup.
        ...
        // Now bring up optional system audio (Core Audio tap), after the mic is live.
