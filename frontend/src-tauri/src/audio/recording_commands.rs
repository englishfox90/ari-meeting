...
pub async fn stop_recording<R: Runtime>(
    ...
    if let Some(task_handle) = transcription_task {
        ...
        match tokio::time::timeout(
            ...
            Err(_) => {
                warn!("⏱️ Transcription timeout (10 minutes) reached, aborting stuck worker to prevent lock deadlock during model unload");
                // Abort the zombie task so it releases any engine lock it may be holding
                // (e.g. a hung native ASR call on silent/empty audio) - otherwise unload_model()
                // below deadlocks waiting for that same lock.
                abort_handle.abort();
            }
        }
        ...
        Some("parakeet") => {
            ...
            if let Some(engine) = engine_clone {
                ...
                match tokio::time::timeout(
                    ...
                    Err(_) => warn!(
                        "⏱️ Parakeet unload timeout (15s) - a stuck worker likely still holds the model lock; continuing shutdown without unloading",
                    ),
                }
            } else {
        ...
        _ => {
            ...
            if let Some(engine) = engine_clone {
                ...
                match tokio::time::timeout(
                    ...
                    Err(_) => warn!(
                        "⏱️ Whisper unload timeout (15s) - a stuck worker likely still holds the model lock; continuing shutdown without unloading",
                    ),
                }
            } else {
    ...
    if let Some((total_duration, active_duration, pause_duration, transcript_segments_count, had_fatal_error, mic_device_name, sys_device_name, chunks_processed)) = analytics_data {
        ...
        .await
        {
            ...
            Ok(_) => None,
            Err(_) => {
                warn!("⏱️ Transcript config timeout (30s) during analytics, continuing shutdown");
                None
            }
        };
        ...
        .await
        {
            ...
            Ok(_) => None,
            Err(_) => {
                warn!("⏱️ Model config timeout (30s) during analytics, continuing shutdown");
                None
            }
        };
