...
#[command]
pub async fn parakeet_load_model(
    ...
    if let Some(parakeet_engine) = parakeet_engine {
...
pub async fn parakeet_download_model<R: Runtime>(
    ...
    if let Some(parakeet_engine) = parakeet_engine {
        ...
        if let Err(e) = parakeet_engine.discover_models().await {
...
pub async fn parakeet_retry_download<R: Runtime>(
    ...
    if let Some(parakeet_engine) = parakeet_engine {
        ...
        {
            let mut active = parakeet_engine.active_downloads.write().await;
        ...
        {
            let mut models = parakeet_engine.available_models.write().await;
        ...
        let _ = parakeet_engine.discover_models().await;

        // Call regular download (emits events)
        parakeet_download_model(app_handle, engine, model_name).await
    } else {
        Err("Parakeet engine not initialized".to_string())
    }
}
