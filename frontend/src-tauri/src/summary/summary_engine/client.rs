...
enum Request {
    Generate {
        ...
        /// Request per-token streaming (see `generate_with_builtin_stream`).
        stream: Option<bool>,
    },
}
...
#[serde(tag = "type", rename_all = "snake_case")]
enum Response {
    /// Incremental token, emitted only while streaming.
    Token { text: String },
...
pub async fn generate_with_builtin(
    ...
    match response {
        ...
        Response::Token { .. } => {
            Err(anyhow!("Unexpected streaming token in non-streaming response"))
        }
    }
}

/// Streaming counterpart of `generate_with_builtin`: enables per-token emission in
/// the sidecar and invokes `on_delta` for each decoded token as it arrives. Returns
/// the full, stop-cleaned text from the terminal response (authoritative — callers
/// should treat the accumulated deltas as cosmetic and use this return value).
pub async fn generate_with_builtin_stream<F>(
    app_data_dir: &PathBuf,
    model_name: &str,
    system_prompt: &str,
    user_prompt: &str,
    ...
    F: FnMut(&str),
{
    ...
    log::info!("Built-in AI streaming generation request (model: {})", model_name);
    ...
    // Terminal outcome captured from inside the line callback.
    let mut terminal: Option<std::result::Result<String, String>> = None;
    let stream_fut = manager.send_request_streaming(request_json, timeout, |line| {
        match serde_json::from_str::<Response>(line) {
            Ok(Response::Token { text }) => {
                on_delta(&text);
                false
            }
            Ok(Response::Response { text, error }) => {
            ...
            // Ignore any non-protocol line (defensive; the sidecar logs to stderr).
            Err(_) => false,
        }
    });
    ...
    match terminal {
        ...
        Some(Err(e)) => Err(anyhow!("Generation failed: {}", e)),
        None => Err(anyhow!("Sidecar closed without a terminal response")),
    }
}
