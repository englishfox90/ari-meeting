...
impl SidecarManager {
    ...
    /// Streaming variant of `send_request`: writes the request once, then reads
    /// response lines and hands each to `on_line` until it returns `true`
    /// (terminal reached). The whole exchange is bounded by `timeout`; on timeout
    /// the sidecar is shut down (mirrors `send_request`). Only one request runs at
    /// a time (RequestGuard), so interleaving of token lines is not a concern.
        ...
        F: FnMut(&str) -> bool,
    {
        ...
        // Write request to stdin (identical to send_request).
        {
            let mut stdin_lock = self.stdin_writer.lock().await;
            let stdin = stdin_lock
                .as_mut()
        ...
        match tokio::time::timeout(timeout, read_loop).await {
            ...
            Ok(Err(e)) => Err(e),
            Err(_) => {
                log::error!("Streaming request timeout after {:?}, shutting down sidecar", timeout);
                if let Err(shutdown_err) = self.shutdown().await {
                    log::error!("Failed to shutdown sidecar after timeout: {}", shutdown_err);
                }
                Err(anyhow!("Request timed out after {:?}", timeout))
