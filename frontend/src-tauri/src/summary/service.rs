...
impl SummaryService {
    ...
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        let api_key = if provider == LLMProvider::Ollama || provider == LLMProvider::BuiltInAI || provider == LLMProvider::CustomOpenAI || provider == LLMProvider::ClaudeCLI || provider == LLMProvider::AppleFoundation {
            // These providers don't require API keys from the standard database column
            // (Claude CLI supplies its own auth via the local `claude` binary).
            String::new()
        } else {
        ...
        let token_threshold = if provider == LLMProvider::Ollama {
            ...
            // transcripts chunk (paired with the processor.rs chunking gate below).
            3500
        } else {
        ...
        match result {
            Ok((final_markdown, english_markdown, num_chunks)) => {
                ...
                .await
                {
                    ...
                    // Record which LLM produced this summary so the UI can show
                    // real provenance (mirrors transcription provenance). Best
                    // effort — a failure here must not fail the summary.
                    if let Err(e) = MeetingsRepository::update_summary_provenance(
                        &pool,
                        &meeting_id,
                        &model_provider,
                        &model_name,
                    ...
                    {
                        warn!(
                            "Failed to record summary provenance for {}: {}",
                            meeting_id, e
                        );
                    }

                    // Record which template produced this summary so the
                    // Template picker reflects it on reopen (including F6
                    // auto-suggested templates). Best effort — a failure here
                    // must not fail the summary.
                    if let Err(e) = MeetingsRepository::update_summary_template(
                        &pool,
                        &meeting_id,
                        &template_id,
                    )
                    ...
                    {
                        warn!(
                            "Failed to record summary template for {}: {}",
                            meeting_id, e
                        );
                    }
                }
