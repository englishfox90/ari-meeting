...
pub enum LLMProvider {
    ...
    BuiltInAI,
    CustomOpenAI,
    /// Uses the user's locally installed Claude Code CLI (`claude`) as a provider.
    ClaudeCLI,
    /// Uses Apple's on-device FoundationModels via the apple-helper sidecar.
    AppleFoundation,
}
...
impl LLMProvider {
    ...
    pub fn from_str(s: &str) -> Result<Self, String> {
        match s.to_lowercase().as_str() {
            ...
            "apple-foundation" => Ok(Self::AppleFoundation),
...
pub async fn generate_summary(
    ...
    // Handle ClaudeCLI provider separately (spawns the local `claude` CLI, no HTTP API)
    if provider == &LLMProvider::ClaudeCLI {
        return crate::summary::claude_cli::generate_with_claude_cli(
            model_name,
            system_prompt,
            user_prompt,
            cancellation_token,
    ...
    // Handle AppleFoundation separately (spawns the apple-helper sidecar, no HTTP API).
    // FoundationModels treats `instruction` as session instructions and `text` as the
    // prompt content, so we map system_prompt → instruction and user_prompt → text.
    if provider == &LLMProvider::AppleFoundation {
        return crate::apple::helper::summarize(
            user_prompt,
            system_prompt,
    ...
    let (api_url, mut headers) = match provider {
        ...
        LLMProvider::AppleFoundation => {
            // This case is handled earlier with early returns
            unreachable!("AppleFoundation is handled before this match statement")
        }
    };
...
fn provider_name(provider: &LLMProvider) -> &str {
    match provider {
        ...
        LLMProvider::AppleFoundation => "Apple Foundation",
    }
}
