...
export function useRecordingStop(
  ...
  const { beginProcessing } = useMeetingProcessing();
    ...
    try {
      ...
      if (isCallApi && transcriptionComplete == true) {
        ...
        try {
          ...
          // If this recording was started from a meeting series, auto-attach the
          // saved meeting to that series. The series-details page stashes the
          // intent in sessionStorage before navigating to the recorder. This must
          // never block or fail the save/processing flow — warn only on error.
          try {
            const pendingSeriesId = sessionStorage.getItem('pendingSeriesLinkId');
            if (pendingSeriesId) {
              sessionStorage.removeItem('pendingSeriesLinkId');
              try {
                await seriesService.link(meetingId, pendingSeriesId);
                // Safety net: the summary pipeline also refreshes the ledger, but
                // fire one here in case auto-summary is off. Fire-and-forget.
                invoke('series_update_ledger', { meetingId }).catch(() => {});
              } catch (error) {
                console.warn('Failed to auto-link meeting to series:', error);
              }
            }
          } catch (error) {
            // sessionStorage unavailable/throwing — never block the save.
            console.warn('Could not read pending series link intent:', error);
          }
          ...
          // Hand off to the background processing pipeline: diarization runs
          // FIRST, then (if auto-summary is on) the summary — so the summary
          // always sees resolved speaker labels. This is non-blocking and
          // mount-independent; the user can navigate away and it keeps running,
          // with per-meeting progress surfaced by MeetingProcessingContext.
          beginProcessing(meetingId, { autoSummary: isAutoSummary });
