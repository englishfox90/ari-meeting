...
interface UseAutoScrollProps {
    ...
    /**
     * Wrapper around the scrolling content. Observed for size changes so we can
     * keep the live line pinned to the bottom even when the current segment
     * grows in place (streaming text) or the virtualizer re-measures — not just
     * when a brand-new segment arrives.
     */
...
export function useAutoScroll({
    ...
    // Keep the live line pinned to the bottom while the CONTENT grows, not just
    // when a new segment appears. During a long meeting the current line streams
    // in place and the virtualizer re-measures taller rows, so a count-only
    // trigger leaves the live line drifting below the fold. A ResizeObserver on
    // the content wrapper fires after layout (with the correct new height), so
    // scrolling to scrollHeight lands exactly on the true bottom. Respects a
    // manual scroll-up via `autoScrollRef` (cleared/restored by the scroll
    // handler above).
    useEffect(() => {
    ...
    }, [contentRef, scrollRef, isRecording, isPaused, disableAutoScroll]);
