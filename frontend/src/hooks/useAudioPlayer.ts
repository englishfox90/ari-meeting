...
export const useAudioPlayer = (audioPath: string | null) => {
  ...
  const loadAudio = async () => {
    ...
    try {
      ...
      // read_audio_file returns raw bytes over the IPC binary channel (an ArrayBuffer).
      const result = await invoke<ArrayBuffer>('read_audio_file', {
        filePath: audioPath
      });
      ...
      console.log('Audio file read, size:', result.byteLength, 'bytes');

      // decodeAudioData consumes (detaches) the buffer; slice to keep a copy.
      const audioData = result.slice(0);
