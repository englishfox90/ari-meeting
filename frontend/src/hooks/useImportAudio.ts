...
export function useImportAudio({
  ...
  // Source path of the in-flight import, so the completion handler can realign the
  // imported meeting's timestamp to the source file's real modified time (calendar
  // matching keys on created_at, which otherwise defaults to upload time).
    ...
    const setupListeners = async () => {
      ...
      const unlistenComplete = await listen<ImportResult>(
        ...
        async (event) => {
          ...
          // Realign created_at to the source file's real modified time so the imported
          // meeting can be matched to its calendar event. Best-effort: an import that
          // saved successfully is never failed by a timestamp-correction hiccup.
          const sourcePath = lastSourcePathRef.current;
          if (sourcePath) {
            try {
              await invoke('set_meeting_time_from_source', {
                meetingId: event.payload.meeting_id,
                sourcePath,
              });
            } catch (error) {
              console.warn('Could not realign imported meeting time to source file:', error);
            }
          }
