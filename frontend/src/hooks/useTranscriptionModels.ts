...
export interface ModelOption {
  provider: 'whisper' | 'parakeet' | 'apple';
  name: string;
  displayName: string;
  size_mb: number;
}
...
export function useTranscriptionModels(transcriptModelConfig: TranscriptModelConfig | undefined) {
  ...
  const fetchModels = useCallback(async () => {
    ...
    // Apple on-device — a single system model (no downloadable file). Offer it
    // only when the on-device Speech stack is actually available (probe-gated,
    // matching TranscriptSettings), so import/retranscribe can route to Apple
    // instead of silently falling back to Whisper.
    try {
      if (isTauriAvailable()) {
        const probe = await probeApple();
        if (probe.speechAvailable) {
          allModels.push({
            provider: 'apple',
            name: '',
      ...
      console.error('Failed to probe Apple on-device availability:', err);
    }
    ...
    // 'apple' has a single (empty-name) system model, so match on provider alone.
    const configuredMatch = allModels.find(
      (m) =>
        (configuredProvider === 'localWhisper' && m.provider === 'whisper' && m.name === configuredModel) ||
        (configuredProvider === 'parakeet' && m.provider === 'parakeet' && m.name === configuredModel) ||
        (configuredProvider === 'apple' && m.provider === 'apple')
    );
