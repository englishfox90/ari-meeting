...
export function deriveRecordingReadiness(input: RecordingReadinessInput): RecordingReadiness {
  if (input.isChecking) {
    return {
      ...
      blockDetail: 'Ari Meeting is reading local devices and model availability.',
    };
  }
  ...
  const microphone: RecordingReadinessItem = input.audioError
    ? {
        ...
        label: 'Microphone',
        state: 'error',
        detail: `Ari Meeting could not read audio devices: ${input.audioError}`,
      }
    : microphoneMatches
      ? {
          id: 'microphone',
    ...
    switch (input.modelState) {
      ...
      case 'error':
        return {
          ...
          label: 'Local transcription',
          state: 'error',
          detail: input.modelError || 'Ari Meeting could not verify the local transcription model.',
        };
      case 'checking':
      default:
        return {
