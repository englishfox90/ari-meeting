...
export function getLocalModelStatus({ provider, model, ollamaModelCount, ollamaError }: LocalModelStatusInput) {
  ...
  if (provider === 'claude-cli' && model) {
    return {
      ...
      description: model === 'default'
        ? 'Claude CLI (local install) is configured for summaries.'
        : `Claude CLI (local install) is configured for summaries — ${model}.`,
    };
  }
  ...
  if (provider === 'apple-foundation') {
    return {
      ready: true,
      description: 'Apple on-device summaries are configured — runs entirely on this Mac.',
    };
  }
