...
import { TranscriptionProviderGlyph } from '../ProviderGlyphs';
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <DialogDescription className="mt-1.5 leading-5">
                  {isProcessing ? progressPresentation?.message || 'Waiting for the first local processing update…' : error && !isProcessing ? 'The original file was not changed. Review the local error below.' : 'Choose one recording. Ari Meeting validates, transcribes, and saves it on this device.'}
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
        ...
        <div className="max-h-[62vh] space-y-4 overflow-y-auto px-6 py-5 sm:px-7">
          {!isProcessing && !error && (
            fileInfo ? (
                    ...
                    <div className="grid gap-4 border-t border-border/70 p-4 sm:grid-cols-2">
                      ...
                      {availableModels.length > 0 && (
                        <div className="space-y-2">
                          ...
                          <Select value={selectedModelKey} onValueChange={setSelectedModelKey} disabled={loadingModels}><SelectTrigger><SelectValue placeholder={loadingModels ? 'Loading models…' : 'Select model'} /></SelectTrigger><SelectContent>{availableModels.map((model) => <SelectItem key={`${model.provider}:${model.name}`} value={`${model.provider}:${model.name}`}><span className="flex items-center gap-2"><TranscriptionProviderGlyph provider={model.provider} className="text-muted-foreground" />{model.displayName} ({Math.round(model.size_mb)} MB)</span></SelectItem>)}</SelectContent></Select>
                        </div>
                      )}
                    </div>
                  )}
              ...
              <div className="rounded-md border border-dashed border-border bg-secondary/25 px-6 py-9 text-center">
                ...
                <p className="mx-auto mt-1 max-w-sm text-sm leading-6 text-muted-foreground">Drag one file anywhere into Ari Meeting, or open the native file picker.</p>
          ...
          {isProcessing && (
            <div className="space-y-4 rounded-md border border-border/80 bg-card p-5">
              ...
              <div className="flex items-start gap-3 border-t border-border/70 pt-4 text-sm leading-6 text-muted-foreground"><ShieldCheckIcon className="mt-0.5 size-4 shrink-0" aria-hidden="true" /><p>Keep Ari Meeting open while the local transcript and meeting record are created.</p></div>
            </div>
          )}
