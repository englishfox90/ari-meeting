...
export function UpdateCheckProvider({ children }: { children: React.ReactNode }) {
  ...
  const { updateInfo, isChecking, checkForUpdates } = useUpdateCheck({
    // No release host configured yet (GitLab TBD). Skip the on-launch check so we
    // don't ping a missing updater endpoint every start; re-enable once releases exist.
    checkOnMount: false,
