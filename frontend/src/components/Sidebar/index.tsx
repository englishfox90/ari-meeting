...
export default function Sidebar() {
  ...
  const { series, isRebuilding } = useSeries();
  // Any series ledger rebuild in flight → subtle pulse on the Series nav item,
  // so a rebuild started on the series-details page stays visible after leaving.
  const anySeriesRebuilding = series.some((s) => isRebuilding(s.id));
  ...
  const isActive = (href: string) => {
    ...
    if (href === '/series') return pathname === '/series' || pathname === '/series-details';
  ...
  const navigationButton = (item: typeof primaryNavigation[number]) => {
    const button = (
      <button
        ...
        className={cn(
          'group relative flex min-h-9 items-center rounded-[5px] text-[13px] font-medium tracking-[-0.01em] transition-colors disabled:cursor-not-allowed disabled:opacity-45',
      ...
      >
        ...
        {item.href === '/series' && anySeriesRebuilding && (
          <>
            <span
              aria-hidden="true"
              className={cn('size-1.5 shrink-0 rounded-full bg-accent motion-safe:animate-pulse', isCollapsed ? 'absolute right-1.5 top-1.5' : 'ml-auto')}
            />
            <span className="sr-only">Rebuilding a series ledger</span>
          </>
        )}
      </button>
    );
  ...
  return (
    <aside
      aria-label="Ari Meeting workspace"
      className={cn(
    ...
    >
      <div className="flex min-w-0 flex-1 flex-col px-3 pb-4 pt-0">
        {/*
         * Draggable strip clearing the macOS traffic lights, which float over
         * the sidebar's top-left in Overlay title-bar mode. Keeps the ARIVO
         * logo from colliding with the window controls and lets the user drag
         * the window from the rail. Height matches the content column strip.
         */}
        <div data-tauri-drag-region aria-hidden="true" className="titlebar h-8 shrink-0" />
              ...
              {meetings.length === 0 ? (
                ...
                <ul className="space-y-0.5">
                  {meetings.map((meeting) => {
                    const processingPhase = processingStates.get(meeting.id)?.phase;
                    const isActiveProcessing = processingPhase === 'diarizing' || processingPhase === 'summarizing';
                    const isProcessingError = processingPhase === 'error';
                    return (
                    <li key={meeting.id} className="group relative">
                      <button
                        ...
                        disabled={isPostProcessing}
                        className={cn(
                          'flex min-h-10 w-full items-center gap-2 rounded-[3px] py-2 pl-3 pr-16 text-left text-[13px] tracking-[-0.01em] text-[hsl(var(--sidebar-muted))] transition-colors hover:bg-[hsl(var(--sidebar-hover))] hover:text-[hsl(var(--sidebar-foreground))] disabled:cursor-not-allowed disabled:opacity-45',
                          currentMeeting?.id === meeting.id && pathname === '/meeting-details' && 'bg-[hsl(var(--accent-soft))] font-medium text-[hsl(var(--sidebar-foreground))]',
                        )}
                      >
                        {(isActiveProcessing || isProcessingError) && (
                          <>
                            <span
                              aria-hidden="true"
                              className={cn(
                                'size-1.5 shrink-0 rounded-full',
                                isProcessingError ? 'bg-destructive' : 'bg-accent motion-safe:animate-pulse',
                              )}
                            />
                            <span className="sr-only">Processing in background</span>
                          </>
                        )}
