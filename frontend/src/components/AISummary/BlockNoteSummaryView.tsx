...
export const BlockNoteSummaryView = forwardRef<BlockNoteSummaryViewRef, BlockNoteSummaryViewProps>(({
  ...
  // Inline "@ref(01:14)" / legacy "[01:14]" timestamp badges for the
  // "markdown" render path below (decoration-only — see
  // summary-ref-badge-plugin.ts). This `editor` instance is only actually
  // mounted via <BlockNoteView> when format === 'markdown'; registering
  // against an unmounted (viewless) editor would throw, so gate on that.
  const player = useAudioPlayback();
  const durationRef = useRef(0);

  useEffect(() => {
    if (!player || format !== 'markdown') return;
  ...
  }, [editor, player?.seekAndPlay, format]);

  // Recompute decorations once duration becomes known / changes (audio loads
  // async, after this editor and plugin are already registered).
  useEffect(() => {
    durationRef.current = player?.duration ?? 0;
    if (format !== 'markdown') return;
    const view = editor._tiptapEditor.view;
    if (view) view.dispatch(view.state.tr);
  }, [editor, player?.duration, format]);
