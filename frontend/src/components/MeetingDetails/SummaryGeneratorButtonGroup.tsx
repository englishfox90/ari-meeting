...
interface SummaryGeneratorButtonGroupProps {
  ...
  /**
   * When the background diarize→summary pipeline is running for this meeting,
   * the manual generate/regenerate action is gated to prevent a second
   * concurrent summary from being kicked off (a race on the same meeting).
   */
  disabled?: boolean;
}
...
export function SummaryGeneratorButtonGroup({
  ...
  // Regenerating an existing summary must go through the dedicated regenerate
  // path (which never re-runs F6 template auto-selection) rather than the
  // first-generation path, so a manually-picked template sticks.
  const triggerSummary = (): Promise<void> =>
    hasSummary && onRegenerateSummary ? onRegenerateSummary() : onGenerateSummary(customPrompt);
    ...
    <ButtonGroup>
      ...
      {isGenerating ? (
        ...
        <Button
          ...
          disabled={isCheckingModels || isModelConfigLoading || disabled}
          title={
            disabled
