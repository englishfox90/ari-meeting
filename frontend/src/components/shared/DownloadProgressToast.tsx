...
export function useDownloadProgressToast() {
  ...
  // Embedding model (Ask Meetings semantic search) — same payload shape as the summary
  // model download; distinct event so it doesn't collide with the summary toast.
  useEffect(() => {
    const unlisten = listen<{
      model: string;
      progress: number;
      ...
      status: string;
      error?: string;
    }>('recall-embedder-download-progress', (event) => {
      const { model, progress, downloaded_mb, total_mb, speed_mbps, status, error } = event.payload;

      const downloadData: DownloadProgress = {
        modelName: model,
        displayName: 'Embedding Model (Nomic)',
        progress: progress ?? 0,
        downloadedMb: downloaded_mb ?? 0,
        totalMb: getDownloadTotalMb(total_mb, model),
        speedMbps: speed_mbps ?? 0,
