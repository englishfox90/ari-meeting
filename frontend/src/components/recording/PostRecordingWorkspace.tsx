      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col justify-center py-4">
        ...
        >
          ...
          <div className="flex items-start gap-3 border-t border-border/70 bg-secondary/35 px-6 py-4 text-sm leading-6 text-muted-foreground sm:px-8">
            ...
            <p>
              {isError
                ? 'Recovery data remains on this device. Keep Ari Meeting open until a supported recovery action is available.'
                : isComplete
                  ? 'The transcript and meeting metadata are saved in Ari Meeting’s local database.'
                  : 'Keep Ari Meeting open. Transcript recovery data remains on this device until the local database save completes.'}
            </p>
          </div>

          {isComplete && (
            <div className="flex items-start gap-3 border-t border-border/70 px-6 py-4 text-sm leading-6 text-[hsl(var(--success))] sm:px-8">
              <CheckIcon className="mt-0.5 size-4 shrink-0" aria-hidden="true" />
              <p>You can keep working — speaker labeling and the summary continue in the background.</p>
            </div>
          )}
        </Surface>
      </div>
