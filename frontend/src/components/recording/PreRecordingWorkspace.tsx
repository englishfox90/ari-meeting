...
interface PreRecordingWorkspaceProps {
  ...
  onStart: (overrideTitle?: string) => Promise<void>;
...
export function PreRecordingWorkspace({
  ...
  const handleStart = async (overrideTitle?: string) => {
    try {
      await onStart(overrideTitle);
    } catch {
  ...
  return (
    <div className="app-page">
      <PageHeader
        ...
        description="Confirm local capture and transcription, then start. Ari Meeting creates the meeting title when recording begins."
        actions={
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
                    Audio, transcript checkpoints, and the saved meeting remain in Ari Meeting&apos;s existing local workflow.
                  </p>
                </div>
              </div>
      ...
      <UpcomingMeetingsPanel onRecord={handleStart} disabled={isStarting} />
      ...
      {hasLifecycleError && (
        <AppState
          ...
          description={statusMessage || 'Ari Meeting could not start the native recording session.'}
          action={
