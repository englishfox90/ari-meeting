...
import { ProviderGlyph } from '@/components/ProviderGlyphs';
import { isSummaryProviderVisible } from '@/lib/summary-provider-visibility';
import { probeApple, AppleProbeStatus } from '@/services/appleService';

export interface ModelConfig {
  provider: 'ollama' | 'groq' | 'claude' | 'claude-cli' | 'apple-foundation' | 'openai' | 'openrouter' | 'builtin-ai' | 'custom-openai';
  model: string;
  whisperModel: string;
  apiKey?: string | null;
...
export function ModelSettingsModal({
  ...
  // Local Claude CLI availability (detected on demand, no fake state)
  const [claudeCliStatus, setClaudeCliStatus] = useState<{
    installed: boolean;
    path: string | null;
    version: string | null;
  } | null>(null);
  const [isDetectingClaudeCli, setIsDetectingClaudeCli] = useState<boolean>(false);

  // Local Apple on-device availability (probed on demand, no fake state)
  const [appleStatus, setAppleStatus] = useState<AppleProbeStatus | null>(null);
  const [isDetectingApple, setIsDetectingApple] = useState<boolean>(false);
  ...
  const modelOptions: Record<string, string[]> = {
    ...
    // "default" = whatever model the local Claude CLI is configured to use.
    'claude-cli': ['default', 'opus', 'sonnet', 'haiku'],
    // FoundationModels is a single on-device system model; "default" is the only option.
  ...
  // Detect the local Claude CLI whenever it is the active provider and we
  // haven't checked yet (e.g. opening settings with claude-cli already saved).
  useEffect(() => {
    if (modelConfig.provider === 'claude-cli' && claudeCliStatus === null && !isDetectingClaudeCli) {
      detectClaudeCli();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [modelConfig.provider]);

  // Probe Apple on-device availability whenever it is the active provider and
  // we haven't checked yet (e.g. opening settings with apple-foundation saved).
  useEffect(() => {
    if (modelConfig.provider === 'apple-foundation' && appleStatus === null && !isDetectingApple) {
      detectApple();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [modelConfig.provider]);
  ...
  // Detect the locally installed Claude CLI (honest status, no fake state)
  const detectClaudeCli = async () => {
    setIsDetectingClaudeCli(true);
    try {
      const status = (await invoke('claude_cli_detect')) as {
        installed: boolean;
        path: string | null;
        version: string | null;
      };
      ...
      console.error('Error detecting Claude CLI:', err);
      setClaudeCliStatus({ installed: false, path: null, version: null });
    } finally {
      setIsDetectingClaudeCli(false);
    }
  };

  // Probe Apple on-device availability (honest status, no fake state)
  const detectApple = async () => {
    setIsDetectingApple(true);
    try {
      const status = await probeApple();
      setAppleStatus(status);
    } catch (err) {
      console.error('Error probing Apple on-device availability:', err);
      setAppleStatus({
    ...
    <div>
      ...
      <div className="space-y-4">
          ...
          <div className="flex space-x-2 mt-1">
            <Select
              ...
              onValueChange={(value) => {
                ...
                // Detect the local Claude CLI when selected
                if (provider === 'claude-cli') {
                  detectClaudeCli();
                }

                // Probe Apple on-device availability when selected
              ...
              <SelectContent className="max-h-64 overflow-y-auto">
                <SelectItem value="builtin-ai">
                  <span className="flex items-center gap-2">
                    <ProviderGlyph provider="builtin-ai" />
                    Built-in AI (Offline, No API needed)
                  </span>
                </SelectItem>
                <SelectItem value="claude-cli">
                  <span className="flex items-center gap-2">
                    <ProviderGlyph provider="claude-cli" />
                    Claude CLI (uses local install)
                  </span>
                </SelectItem>
                <SelectItem value="apple-foundation">
                  <span className="flex items-center gap-2">
                    <ProviderGlyph provider="apple-foundation" />
                    Apple (on-device)
                  </span>
                </SelectItem>
                <SelectItem value="ollama">
                  <span className="flex items-center gap-2">
                    <ProviderGlyph provider="ollama" />
                    Ollama
                  </span>
                </SelectItem>
                {isSummaryProviderVisible('claude', modelConfig.provider) && (
                ...
                {isSummaryProviderVisible('custom-openai', modelConfig.provider) && (
                  <SelectItem value="custom-openai">
                    <span className="flex items-center gap-2">
                      <ProviderGlyph provider="custom-openai" />
                      Custom Server (OpenAI)
                    </span>
                  </SelectItem>
                )}
                {isSummaryProviderVisible('groq', modelConfig.provider) && (
                  <SelectItem value="groq">
                    <span className="flex items-center gap-2">
                      <ProviderGlyph provider="groq" />
                      Groq
                    </span>
                  </SelectItem>
                )}
                {isSummaryProviderVisible('openai', modelConfig.provider) && (
                  <SelectItem value="openai">
                    <span className="flex items-center gap-2">
                      <ProviderGlyph provider="openai" />
                      OpenAI
                    </span>
                  </SelectItem>
                )}
                {isSummaryProviderVisible('openrouter', modelConfig.provider) && (
                  <SelectItem value="openrouter">
                    <span className="flex items-center gap-2">
                      <ProviderGlyph provider="openrouter" />
                      OpenRouter
                    </span>
                  </SelectItem>
                )}
        ...
        {modelConfig.provider === 'claude-cli' && (
          <div className="rounded-[10px] border bg-muted/30 p-3 text-sm">
            {isDetectingClaudeCli || claudeCliStatus === null ? (
              <span className="flex items-center gap-2 text-muted-foreground">
                <ArrowPathIcon className="h-4 w-4 animate-spin" />
                Checking for a local Claude CLI…
              </span>
              ...
              <div className="space-y-1">
                ...
                {claudeCliStatus.path && (
                  <p className="truncate text-xs text-muted-foreground">{claudeCliStatus.path}</p>
                )}
                <p className="text-xs text-muted-foreground">
                  Summaries run through your locally installed <code>claude</code> using its own
                  sign-in — no API key needed. Pick a model below (&quot;default&quot; uses the CLI&apos;s
                  configured model).
                </p>
              </div>
            ) : (
              <div className="space-y-1">
                <span className="font-medium text-foreground">Claude CLI not found</span>
                <p className="text-xs text-muted-foreground">
                  Install Claude Code and make sure <code>claude</code> is on your PATH, then re-check.
                </p>
                <Button variant="outline" size="sm" className="mt-1" onClick={detectClaudeCli}>
                  Re-check
                </Button>
              </div>
            )}
        ...
        {/* Apple On-Device Status Section */}
        {modelConfig.provider === 'apple-foundation' && (
          <div className="rounded-[10px] border bg-muted/30 p-3 text-sm">
            {isDetectingApple || appleStatus === null ? (
              <span className="flex items-center gap-2 text-muted-foreground">
                <ArrowPathIcon className="h-4 w-4 animate-spin" />
                Checking on-device availability…
              </span>
            ) : appleStatus.foundationAvailable ? (
              <div className="space-y-1">
                <span className="flex items-center gap-2 font-medium text-foreground">
                  <CheckIcon className="h-4 w-4" />
                  Apple on-device summaries ready — runs entirely on this Mac, no API key
                </span>
                <p className="text-xs text-muted-foreground">
                  This is a compact on-device model. Summaries are fast and fully private, but shorter
                  and less detailed than cloud models, and it may not follow complex formatting (such as
                  timestamp tables) reliably. Best for quick, offline summaries.
                </p>
              </div>
            ) : (
              <div className="space-y-1">
                <span className="font-medium text-foreground">Apple on-device summaries not available</span>
                <p className="text-xs text-muted-foreground">
                  {!appleStatus.osOk
                    ? 'Requires macOS 26+'
                    : !appleStatus.appleIntelligence
                      ? 'Turn on Apple Intelligence in System Settings'
                      : appleStatus.error || 'Not available on this Mac'}
                </p>
                <Button variant="outline" size="sm" className="mt-1" onClick={detectApple}>
                  Re-check
                </Button>
              </div>
            )}
