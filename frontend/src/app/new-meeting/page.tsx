...
export default function NewMeetingPage() {
  ...
  // Home page's calendar shortcut (see UpcomingMeetingsPanel) can't start a
  // recording itself — that machinery only mounts here — so it hands off the
  // clicked event's title via sessionStorage and this page starts it the
  // moment it's ready. Clicking "Record" on the home page IS the user's
  // explicit consent; this just finishes what that click already started.
  const pendingCalendarTitleHandled = useRef(false);
  useEffect(() => {
    if (pendingCalendarTitleHandled.current || !showPreRecording) return;
    const pendingTitle = sessionStorage.getItem('pendingCalendarRecordTitle');
    if (!pendingTitle) return;
    pendingCalendarTitleHandled.current = true;
    sessionStorage.removeItem('pendingCalendarRecordTitle');
    void handleRecordingStart(pendingTitle);
  }, [showPreRecording, handleRecordingStart]);
