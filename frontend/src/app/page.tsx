...
import { UpcomingMeetingsPanel } from '@/components/recording/UpcomingMeetingsPanel';
...
export default function DashboardPage() {
  ...
  // Clicking Record on a calendar event is the user's explicit consent to
  // start capturing it — same as the equivalent panel on the New Meeting
  // screen. The home page doesn't itself host the recording-start machinery
  // (that lives with the RecordingStateContext-aware New Meeting page), so we
  // hand off the chosen title via sessionStorage and let that page's own
  // mount effect start the recording immediately.
  const handleRecordFromCalendar = async (overrideTitle?: string) => {
    if (overrideTitle) {
      sessionStorage.setItem('pendingCalendarRecordTitle', overrideTitle);
    }
  ...
  return (
    <div className="app-page">
      ...
      <section
        ...
        className={`mt-12 grid gap-x-10 gap-y-10 ${showCalendarColumn ? 'lg:grid-cols-2' : 'grid-cols-1'}`}
      >
        <div>
          ...
          {recentMeetings.length === 0 ? (
            <AppState
              ...
              description="Your first completed recording will appear here. Ari Meeting does not add sample meetings or fabricated activity."
        ...
        <div className={showCalendarColumn ? '' : 'hidden'}>
          <UpcomingMeetingsPanel
            onRecord={handleRecordFromCalendar}
            onVisibilityChange={setShowCalendarColumn}
            className="mt-0"
          />
        </div>
      </section>
