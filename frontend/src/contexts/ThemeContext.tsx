...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  const [preference, setPreference] = useState<ThemePreference>(nativeQaTheme ?? 'dark');
  const [resolvedTheme, setResolvedTheme] = useState<'light' | 'dark'>(nativeQaTheme ?? 'dark');

  useEffect(() => {
    if (nativeQaTheme) return;
    const stored = window.localStorage.getItem(storageKey);
    const nextPreference: ThemePreference = stored === 'light' || stored === 'dark' || stored === 'system' ? stored : 'dark';
