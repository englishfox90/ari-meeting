...
import { MODEL_TIERS, recommendTier, type ModelTier } from '@/lib/model-tiers';
import { probeApple, isTauriAvailable } from '@/services/appleService';
...
interface OnboardingContextType {
  ...
  // Model tiers (3-tier onboarding selection)
  selectedTier: ModelTier;
  recommendedTier: ModelTier;
  appleEligible: boolean;
  appleAssetsInstalled: boolean;
  // Permissions
...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  // Model tier selection (Express / Balanced / Demanding).
  const [selectedTier, setSelectedTierState] = useState<ModelTier>('balanced');
  const [recommendedTier, setRecommendedTier] = useState<ModelTier>('balanced');
  const [appleEligible, setAppleEligible] = useState(false);
  const [appleAssetsInstalled, setAppleAssetsInstalled] = useState(false);
  // Tracks whether the user has explicitly picked a tier, so the async
  // recommendation never overwrites a deliberate choice.
  const tierChosenByUserRef = useRef(false);

  const setSelectedTier = useCallback((tier: ModelTier) => {
  ...
  // Compute the recommended tier from REAL signals: Apple on-device eligibility
  // (probe) + installed RAM. Honest on failure — defaults to Balanced.
  useEffect(() => {
    let cancelled = false;
    ...
    const detectRecommendation = async () => {
      ...
      if (isTauriAvailable()) {
        try {
          const probe = await probeApple();
          eligible = probe.speechAvailable && probe.foundationAvailable;
          assetsInstalled = probe.speechAssetsInstalled;
        } catch (error) {
          console.warn('[OnboardingContext] Apple probe failed:', error);
        }

        try {
          const resources = await invoke<{ total_memory_mb?: number }>('get_system_resources');
          if (resources && typeof resources.total_memory_mb === 'number') {
            ramGb = resources.total_memory_mb / 1024;
          }
        } catch (error) {
          console.warn('[OnboardingContext] get_system_resources failed:', error);
        }
      }

      if (cancelled) return;

      const rec = recommendTier(ramGb, eligible);
      setAppleEligible(eligible);
      setAppleAssetsInstalled(assetsInstalled);
      setRecommendedTier(rec);
      console.log('[OnboardingContext] Tier recommendation:', { ramGb, eligible, rec });

      // Only seed the selection if the user has not already chosen a tier.
      if (!tierChosenByUserRef.current) {
        setSelectedTierState(rec);
      }
    };
  ...
  const completeOnboarding = async () => {
    try {
      ...
      // Persist the selected tier's transcription + summary providers/models.
      const tier = MODEL_TIERS[selectedTier];
      const summaryModelToSave = tier.summary.model;

      await withTimeout(
        invoke('complete_onboarding', {
          model: summaryModelToSave,
          summaryProvider: tier.summary.provider,
          transcriptionProvider: tier.transcription.provider,
          transcriptionModel: tier.transcription.model,
        }),
        'Ari Meeting could not save setup. Please try again.',
        8_000,
      );
      setCompleted(true);
      console.log('[OnboardingContext] Onboarding completed with tier:', selectedTier);
      ...
      // Only builtin-ai tiers have a downloadable local summary model to prepare.
      // Express (apple-foundation) uses Apple's on-device model — nothing to fetch.
      if (tier.summary.provider === 'builtin-ai') {
        setSelectedSummaryModel(summaryModelToSave);
        ...
        void withTimeout(
          ...
          if (!selectedModelReady) {
            requestSummaryModelDownload(summaryModelToSave);
          }
        }).catch((error) => {
