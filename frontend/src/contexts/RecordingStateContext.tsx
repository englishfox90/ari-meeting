    ...
    const setupListeners = async () => {
      try {
        ...
        const unlistenStopped = await recordingService.onRecordingStopped((payload) => {
          ...
          setState(prev => {
            // Never regress a status that has already advanced past STOPPING
            // (e.g. SAVING/COMPLETED) — a late/duplicate 'recording-stopped'
            // event must not clobber forward progress.
            if (STATUS_RANK[prev.status] >= STATUS_RANK[RecordingStatus.STOPPING]) {
              return {
                ...prev,
                isRecording: false,
                isPaused: false,
        ...
        const unlistenShutdownProgress = await recordingService.onRecordingShutdownProgress((payload) => {
          ...
          const nextStatus = update.phase === 'processing'
            ? RecordingStatus.PROCESSING_TRANSCRIPTS
            : RecordingStatus.STOPPING;

          setState(prev => {
            // Ignore native shutdown-progress events that arrive after the
            // frontend save flow has already advanced past this stage (e.g.
            // SAVING/COMPLETED) — they must never regress the UI backward.
            if (STATUS_RANK[prev.status] > STATUS_RANK[nextStatus]) {
              return prev;
            }
