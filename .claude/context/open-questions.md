...
- **`prose` styling bug + frontend config dedup** — ✅ **Done (verified 2026-07-17):** `tailwind.config.js` now registers `@tailwindcss/typography` (`plugins` array), so markdown panels render styled (`prose` usage has since moved to `src/app/series-details/page.tsx` and `src/components/ask/AskMessage.tsx`). The duplicate configs are gone — `postcss.config.mjs`, `eslint.config.mjs`, and `tailwind.config.ts` are all deleted; the authoritative set is `tailwind.config.js` / `postcss.config.js` / `.eslintrc.json`. The `.ts`'s optional `fontSize` scale was **not** carried over (Tailwind defaults are in use). Reminder for future edits: `tailwind.config.js` changes must stay in lockstep with `DESIGN.json`/`DESIGN.md` (visual-system test).
