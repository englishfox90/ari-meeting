...
- **Additive and non-destructive by default.** New capability = new table or new nullable column + a new registered migration. Never edit a shipped migration; never drop/retype a column with live data without a data-migration path and explicit sign-off. (Mirrors the Rust `migration-writer` agent's discipline.) **`v1_baseline` is FROZEN as of the 2026-07-22 incident** — it was extended in place while "unshipped," which (with `eraseDatabaseOnSchemaChange` on) silently wiped a real 22-meeting DB. It is now shipped: all changes are new `v2+` migrations, and the erase flag is banned by default (`docs/plans/robust-migration-and-backup.md`).
